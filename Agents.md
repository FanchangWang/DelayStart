...
| 要改什么 | 去哪 |
|---|---|
| 调度计划（过滤 / 排序 / 到点计算） | `Services/SchedulePlan.cs`（🔴 `BuildWithSkipped` 才是完整判定：`Entries` = 计划、`SkippedToday` = 今天不在周期内的启用条目，调度端靠后者写调度日志 FR-15.26；`Build` = 取 `.Entries`） |
...
| `D-x` | 决策点（D1–D117） | `docs/decisions.md` | `D17` |
...
| `docs/decisions.md` | 每个决策的结论与取舍（D1–D117）+ R1–R13 风险去向 | 出现新的取舍（追加编号），或推翻旧决策（并入取代它的条目，**编号保留**） |
...
- ❌ 在 XML 注释里用 `<see cref="…"/>` 指向**当前编译单元不可见**的类型 / 成员（跨程序集的 private / internal、测试项目引用不到的 App 类型、同类私有成员的跨类型引用）—— `TreatWarningsAsErrors` 下 CS1574 直接构建失败且报错位置在注释里不好认。跨程序集 public 类型用全限定名（`<see cref="DelayStart.Core.Models.RunRecord"/>`），不可达的一律降级为 `<c>类型.成员</c>` 纯文本（D116 一轮连踩三次）。
- ❌ 把"有条件地什么都不做"做成**日志空白**（周期不匹配 = 不上报、不统计、不通知）—— 功能正确但行为不可解释：用户周六登录看到一片安静，调度日志里连一行都没有。现在跳过的条目写 `Skipped` + 「今天不在启动周期内，未启动」，**全部跳过时也写一份归档 + `current-run.json`**（FR-15.26 / D89 / `pitfalls.md` 十八）。⚠️ 同时：`_items`（运行侧）与 `_record.Items`（日志侧）别再靠下标对齐，先建局部 `planned` 列表两边各自引用。
...
- ❌ 拿"这个**句柄值**我见过"当幂等依据（按 HWND 记账的静态表）—— HWND 会被系统回收复用给新窗口，复用后表里那条记录属于**已销毁的窗口**，判据必须当场核对"现在的状态对不对"（`GetWindowLongPtrW(hwnd, GWLP_WNDPROC)` 是不是自己的过程），而不是"我做过没有"。实测形态：新窗口**静默失去** `WM_DROPFILES` 转发（`DragAcceptFiles` 在早退前已执行，消息照发、落进默认过程，界面上是"拖进去没反应"，零日志零异常）。⚠ `WindowSizing.EnforceMinimum` 里那一份长得一样但**不能动** —— 它防的是两个子类化器互相压栈成环，见 `pitfalls.md` 二十 / D106。
- ❌ 重扫快照时**只按来源剔一部分数据** —— `RescanSourceAsync` 里 `entries` / `failures` / `pixels` 三类必须**同进同退**：漏掉 `pixels` 会让旧来源的条目（已经不在快照里）永远留在字典键上，而且每轮重扫再整份复制一遍，于是"刷新本页"点一次就留下该来源条目数那么多条（2026-09-24 批复 28 / D107）。
- ❌ 把 ViewModel 里"值其实是常量"的成员写成 `public string X => "常量";` —— 分析器报 CA1822（建议 static），而 `x:Bind` **绑不到静态属性**，两边都不满足。写成**带初始值的自动属性**（`{ get; } = …`）：有后备字段不触发分析器，又是实例成员（2026-09-24 批复 28 / `pitfalls.md` 二十一）。
