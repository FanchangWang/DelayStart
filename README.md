...
<details>
<summary><strong>更多界面</strong>（四个自启动来源页 · 运行日志 · 设置）</summary>
...
- **失败可见** —— 每个条目的启动结果、耗时、失败原因都落盘，运行日志里逐条可查。
...
**运行中会发生什么**：登录 3 秒后调度进程被拉起，托盘出现一个小图标，鼠标悬停能看到"已启动 3/8 · 下一项 12 秒"。跑完之后它会按「通知策略」发一条 Windows 系统通知（默认只有失败才发），然后自动退出。想看实时进度，点一下托盘图标即可弹出进度面板。完成情况任何时候都能在「运行日志」页查。
...
| 日志 / 实时状态 / 运行归档 | `%LOCALAPPDATA%\DelayStart\logs\` · `state\` · `runs\` · `guard\` |
...
能。「从不通知」下守卫照样扫描、照样把被改回启用的项重新禁用，只是不发通知。想看它做过什么，看 `%LOCALAPPDATA%\DelayStart\logs\guard.log` 就行。
...
**设置 → 调度 → 通知策略**：默认「有失败时通知」（全成功不打扰），也可选「总是通知」或「从不通知」。它是 Windows 系统通知（和守卫的提醒一个形态），点一下直达运行日志页。进度面板与通知互不影响：想看实时进度随时点托盘图标弹出面板，面板关没关都不影响通知照常发。
