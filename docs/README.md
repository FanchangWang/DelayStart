...
10. **AI 只生成 commit message，不自动执行 `git commit`**（用户明确要求时除外）。
...
- **新增决策点** → 在 `requirements.md` 第十一节与本文「决策点状态」两处登记。
