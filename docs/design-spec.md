# DelayStart — 界面设计与文案

> 状态：**待评审**。评审通过后才进入编码。
> 配套产物：`docs/api-analysis.md`（Demo 分析结论）、`docs/ui-mockup.html`（可交互原型）

---

## 一、产品定位

| 项 | 内容 |
|---|---|
| 应用名 | **DelayStart**（唯一名，全英文 —— D61 改判，见下） |
| 中文描述名 | 延时启动管理器（**仅文档用**，不再作为应用名出现） |
| 图标 | 源图 `assets\icon\delay.png`（蓝色描边时钟 + 青色沙漏，透明底）→ `tools/make-icon.py` 生成**三份** 10 档 ico（管理端 / 调度端 / 调度端告警角标）。明暗背景下都完整可读；窗口、托盘、快捷方式、安装程序与「设置 → 应用」用同一张图。生成方式见 `build-and-test.md` 7.6 |
| 一句话 | 把开机时一股脑抢占资源的自启程序，改成按你的节奏、错峰、延时启动。 |
| 副标语 | 不删除任何原始配置 · 全部可一键还原 |
| 目标用户 | Win11 用户，开机被 20+ 个自启程序拖慢，但又不舍得直接禁用某个程序 |
| 核心价值 | ① 全位置扫描（注册表 3 处 + 2 个启动文件夹 + 计划任务 + UWP）<br>② 软禁用，随时可还原<br>③ 延时可编排，处理程序间的启动依赖 |
| 与"直接禁用自启"的区别 | 禁用 = 永远不启动；延时启动 = **照常启动，只是晚一点**。适合"我需要它，但不想它开机就跟我抢资源" |

> 🔴 **D61（2026-09-21）改判 D6**：原方案是双名（中文名「延时启动管理器」+ 英文名 `DelayStart`）。
> 用户实测后要求**应用名只用英文 `DelayStart`** —— 双名会让"开始菜单一项、任务栏一项、
> 「应用和功能」第三项"三处对不上。中文名退回文档，只做产品解释。
> 落地范围：窗口标题（`MainWindow.xaml` 的 `Window.Title` / `TitleBar.Title`）、
> 安装器的 `AppName` / `AppVerName` / 快捷方式名、CLI 帮助抬头、计划任务描述。

**明确的非目标（不做）**：
- ❌ 不做启动项清理/优化评分（那是另一个产品）
- ❌ 不做系统服务延时接管（服务用系统原生的「自动（延迟启动）」）
- ❌ 不碰内核驱动启动顺序
- ❌ 不修改 Winlogon / Userinit / Shell（只读展示）

---

## 二、信息架构

左侧 `NavigationView`，共 6 个顶层模块 + 「自启动项」下的 5 个子视图。

```
┌ DelayStart ────────────────────────────────────────┐
│                                        [☀/🌙] [—]  │
├────────────────┬───────────────────────────────────┤
│ ⌂  总览         │                                   │
│ ▤  自启动项  13 │                                   │
│    ├ 全部       │          内容区                    │
│    ├ 注册表   6 │                                   │
│    ├ 启动文件夹 2│                                   │
│    ├ 计划任务  3│                                   │
│    └ UWP 应用  2│                                   │
│ ⏱  延时启动   4 │                                   │
│ 🛡  系统启动项   │                                   │
│ ▤  运行日志     │                                   │
│ ⚙  设置         │                                   │
├────────────────┴───────────────────────────────────┤
│ 调度任务：已创建 · 登录后 3 秒触发        [详情]     │
└────────────────────────────────────────────────────┘
```

导航栏为什么把「来源」做成子项：这个软件的差异化就是「扫得全」。把 5 个来源平铺在导航上，用户一眼就知道程序查了哪些地方、各自查到几个 —— 比藏在筛选器里强。

### 搜索框为什么从导航栏挪进页面

**原始设计是错的。** 导航栏顶部放一个「搜索启动项」输入框，效果是：用户在「延时启动」页打字，页面被强制跳转到「自启动项」页 —— 用户的上下文被抢走。

改成**页面内搜索**，两处各自独立、各搜各的：

| 位置 | 搜什么 | 匹配字段 |
|---|---|---|
| 自启动项页工具栏 | 系统里已有的自启动项 | 名称 / 路径 / 命令行参数 / 值名或文件名 / 所在位置 / 来源类型 |
| 延时启动页工具栏 | 已接管的条目（含手动添加） | 同上 |

规则：
- 只过滤**当前页**的列表，绝不跳页、绝不改 `state.src`
- 搜索状态与页内筛选器（状态 / 排序）叠加生效，互不干扰
- 输入框有内容时右侧显示清空按钮；清空后恢复完整列表
- 「延时启动」页处于搜索状态时，「上移 / 下移」按钮禁用（顺序是全局语义，在过滤视图中调整会产生歧义），并在表下给出说明
- 空结果时给「清空搜索」按钮，不是干巴巴一句"没有数据"

导航栏顶部不再有任何输入控件。

---

## 三、逐页 UI 文案

### 页面 1 · 总览

**标题**：总览
**副标题**：开机自启动一览，以及延时启动的执行计划

**统计卡（4 张）**

| 卡片标题 | 数值示例 | 辅助文案 |
|---|---|---|
| 已扫描自启动项 | 15 | 其中 2 项已禁用 · 4 项为系统组件 |
| 已接管延时启动 | 5 | 系统自启动 4 项 · 手动添加 1 项 |
| 开机加载窗口 | 1 分 00 秒 | 从登录到最后一个程序启动 |
| 开机调度任务 | 已就绪 | 登录后 3 秒触发 · 以最高权限运行 |

> **计数口径**：`已扫描自启动项` 只数系统里扫到的条目（不含手动添加）；`已接管延时启动` 数所有被接管的条目，并拆出手动添加的数量。两个数字口径不同是刻意的 —— 第一个回答"系统里有什么"，第二个回答"我接管了什么"。

**「调度任务」卡片**
- 已就绪状态：`调度任务已创建` / `触发器：登录时，延迟 3 秒` / `运行身份：最高权限（不弹 UAC）`
- 缺失状态（警示色）：
  > ⚠ **尚未创建开机调度任务**
  > 延时启动不会生效。需要创建计划任务，在登录后唤起调度器。
  > [立即创建] [稍后再说]

**「最近调度」卡片**
小标题：最近一次开机调度
- 记录行：`✅ 微信 · 30s · 成功` / `❌ pot 翻译工具 · 60s · 启动失败（文件不存在）`
- 无记录时：`暂无调度记录，重启后在此查看`

**操作区**：`[刷新扫描]` `[立即模拟调度]`

> **「立即模拟调度」是刻意放在首屏的**。延时配置写完必须重启才能验证，体验很差。这个按钮让用户当场看到效果（0.5 倍速播放**进度视图** + 真的启动程序），是开发调试和用户验收的双赢。

---

### 页面 2 · 自启动项

**标题**：自启动项
**副标题**：共 13 项 · 已接管 3 项 · 已禁用 1 项

**工具栏**（从左到右）
`[🔍 搜索名称 / 路径 / 值名]` `[来源 ▾]` `[状态 ▾]` `排序 ▾` … `[刷新扫描]`
批量模式下额外出现：`已选 3 项` `[批量延时启动]` `[批量禁用]` `[取消选择]`

> **「手动添加」按钮不在这里** —— 自启动项页的职责是「列出系统里已有的东西」，不是创造新东西。手动添加只存在于「延时启动」页。
>
> 由此引出一条数据边界：**手动添加的条目不进入本页**。它没有注册表值可以软禁用，也没有来源可以恢复，混进本页会让「禁用」这个动作变得无意义。本页的提示条里写明了这一点。

**筛选器文案**
- 来源：`全部来源` / `注册表` / `启动文件夹` / `计划任务` / `UWP 应用`
- 状态：`全部状态` / `已启用` / `已禁用` / `已接管` / `已失效`
- 排序：`按来源` / `按名称` / `按状态`

**列表行（示例）**

```
┌───────────────────────────────────────────────────────────────────────┐
│ [W图标] 微信                                      ● 已接管    [⋯]    │
│         C:\Program Files\Tencent\Weixin\Weixin.exe  -autorun          │
│         注册表 · HKEY_CURRENT_USER\...\Run · 值名 Weixin               │
│         延时 30 秒 · 普通用户                [移出延时]               │
├───────────────────────────────────────────────────────────────────────┤
│ [☁图标] OneDrive                                  ● 已启用    [⋯]    │
│         C:\Program Files\Microsoft OneDrive\OneDrive.exe  /background │
│         启动文件夹 · 用户启动文件夹 · OneDrive.lnk                      │
│                                     [延时启动]  [禁用]                │
├───────────────────────────────────────────────────────────────────────┤
│ [⚙图标] Adobe Acrobat Update Task                ○ 已禁用    [⋯]    │
│         计划任务 · \Adobe Acrobat Update Task · 登录时触发             │
│         启动身份 普通用户                                     [启用]  │
└───────────────────────────────────────────────────────────────────────┘
```

**状态徽标文案与配色**

| 徽标 | 含义 | 配色 |
|---|---|---|
| `已启用` | 系统会正常自启 | 中性灰描边 |
| `已禁用` | 已通过软禁用标记阻止自启 | 警示黄 |
| `已接管` | 原自启已禁用，改由本程序延时启动 | 强调色实心 |
| `已失效` | 路径不存在 / 任务已被删除 | 危险红 |
| `受保护` | 系统组件，不可修改 | 灰色 + 锁图标 |

**内嵌说明文案**（列表顶部常驻一条轻量提示条）
> 这里的「禁用」与「移出延时」都是**软禁用**：不会删除注册表值，也不会移动文件，随时可以还原。<br>
> 本页只列出**系统里已经存在**的自启动项。手动添加的程序属于调度任务而非系统自启动项，只出现在「延时启动」页。

**行内操作菜单 `⋯`**
- `打开文件位置`
- `复制命令行`
- `查看来源详情`（弹出：完整注册表路径 / 任务 XML / AUMID / 快捷方式目标）
- `在任务管理器中查看`
- →（已接管项）`编辑延时设置`

**空状态**
- 筛选无结果：`没有匹配的启动项` / `试试更换筛选条件或清空搜索关键词`
- 该来源本就为空：`此位置没有自启动项` / `注册表 ...\Run 下没有找到任何条目`

---

## 三之二、延时配置编辑器（弹窗）

「自启动项」页的 `[延时启动]`、「延时启动」页的 `[手动添加]` 和 `[编辑]`，**共用同一个弹窗组件**，靠"是否已绑定系统来源"分叉。这样用户只需要理解一套交互。

### 4 种进入方式 → 3 种形态

| 入口 | 目标程序 | 可编辑字段 | 底部主按钮 |
|---|---|---|---|
| 自启动项页 `[延时启动]` | **只读**（由系统项绑定） | 参数 / 延时 / 身份 | 加入延时启动 |
| 延时启动页 `[编辑]`（系统来源） | **只读** | 参数 / 延时 / 身份 | 保存修改 |
| 延时启动页 `[编辑]`（手动条目） | 可更换（「程序」/「UWP 应用」两个页签） | 名称 / 参数 / 工作目录 / 延时 / 身份 | 保存修改 |
| 延时启动页 `[手动添加]` | 可选择（同左） | 名称 / 参数 / 工作目录 / 延时 / 身份 | 加入延时启动 |

分叉的唯一依据是**这条记录有没有系统来源**。有来源 → 路径由系统项决定，不允许改；没来源 → 路径是这个记录的全部意义，必须能改。

> 原第 4 块「接管对象 / 系统影响」已按 **D57** 整块删除（对手动条目是固定不变的同一段话；系统条目的那部分交代由弹窗副标题承担）。

### 布局

```
┌──────────────────────────────────────────────────┐
│ 手动添加延时启动                                 │
│ 选择一个程序，由本程序在登录后延时启动           │
│                                                  │
│ 目标程序                                         │
│ 「程序」页签选磁盘上的文件，「UWP 应用」页签挑应用│
  ┌────────────────────────────────────────────────┐
  │ [ ● 程序 ]  [ ○ UWP 应用 ]                     │
  │                                                │
  │ 拖入文件，或点击选择程序                       │
  │ 支持 .exe / .lnk / .bat / .cmd / .ps1          │
  │                                                │
  │ 显示名称 — 自动取自文件名，可改                │
  │ [ Ditto 剪贴板                             ]   │
  │ 命令行参数 — 可选                              │
  │ [                                          ]   │
  │ 工作目录 — 可选                                │
  │ [                                          ]   │
  └────────────────────────────────────────────────┘
│                                                  │
│ 延时时间                                         │
│ 以登录时刻为起点的绝对时间点，不是累加           │
  ┌────────────────────────────────────────────────┐
  │ (•)立即  ( )5 秒  ( )10 秒  ( )15 秒           │
  │ ( )20 秒  ( )30 秒  ( )1 分钟                  │
  │ 自定义(秒) [ 30 ]  手动输入的自定义值提交前确认│
  └────────────────────────────────────────────────┘
│                                                  │
│ 启动身份                                         │
  ┌────────────────────────────────────────────────┐
  │ (•)普通身份   ( )管理员                        │
  │ 以当前登录用户身份启动，拖拽、剪贴板都正常     │
  └────────────────────────────────────────────────┘
│                                                  │
│                                  [取消][加入]    │
└──────────────────────────────────────────────────┘
```

### 设计要点

- **每个功能块 = 子标题 + 一张边框卡片（D56）**：子标题是纯文本（14px / SemiBold，与设置页 `SectionHeaderStyle` 同一视觉），卡片用默认卡片画刷与圆角。**不再写数字序号** —— 弹窗只有一条主线，序号不提供额外信息，子标题本身就是分节标识。原来的弹窗是一堆无标题的控件堆叠，看不出结构。
- **目标程序块顶部是页签（D53 / D56）**。「程序」= 磁盘上的文件（拖入 / 文件选择器），「UWP 应用」= 已安装的应用（只能从应用清单里挑）。**页签就是"目标属于哪一类"这个状态本身** —— 两个页签各存各的目标值，`TargetPath` 按当前页签取值，所以不存在"两类目标互相覆盖同一个字段"的问题（D48 那套"换了 UWP 还得能换回普通程序"从结构上消失）。
  - 页签形态是**两颗等宽的分段按钮**（`ToggleButton`），选中那颗是 **accent 实心填充** —— 与延时 / 身份胶囊同一套"选中 = 高亮"语言。🔴 早期用的 `SelectorBar` 选中态只是文字下方一小段指示条，在弹窗这种小面积里几乎看不出来（用户 2026-09-20 反馈）。
  - 🔴 `ToggleButton` 默认允许"再点一下取消选中"，必须把 `Unchecked` 顶回去，否则会出现"两颗都没选中、面板却还停在那页"的矛盾状态。
- **两个页签各自摆该有的字段（D59 修正后）**：「程序」页签 = 拖放区 + 显示名称；「UWP 应用」页签 = 选择入口 + 已选应用卡片 + 一行说明。**命令行参数与工作目录摆在页签之外**（目标卡片的直接子项）—— 因为**系统条目根本没有页签**，把这两项放进页签面板会让它们跟着面板一起被折叠（D53 引入的缺陷：接管系统条目时这两项**静默消失**）。可见性规则收敛在一处（`ApplyTargetFieldVisibility`）：名称仅"手动条目 + 「程序」页签"；**参数与工作目录除 UWP 外一律显示**（UWP 参数不转发、也没有工作目录可言 —— 摆着只会让人以为填了有用）。
- **目标选择是大控件**：拖放区 / 应用选择入口各占整块，选中后原地变成「名称 + 路径 + [更换]」，视觉上"填好了"这件事一目了然。卡片内这些子区域改用 `SubtleFillColorSecondaryBrush` 浅底、**不套第二层描边**，避免框套框。
- **工作目录不自动填（D58）**：选完程序后工作目录**保持用户原本的内容**（默认即空）。"留空"本身有语义（用程序所在目录），自动填一个同样的值只是把"没设置"变成"设置成了同一个值"，还会在换程序之后留下一个陈旧路径。
- **原第 4 块已整块删除（D57）**：`接管对象 / 系统影响` 对手动条目是固定不变的同一段话（不提供新信息），对系统条目的那份交代已由弹窗副标题 `加入后原自启动项将被软禁用` 承担。
- **底部活摘要已删除（D53）**：`将在 登录后 30 秒 以 普通身份 启动` 是上方两个胶囊的复述，同一屏内没有新信息。
- **弹窗宽度不覆盖（D55）**：用框架默认 `ContentDialogMaxWidth = 548`（内容区，WinUI `generic.xaml`；标题区最多 56、内容区内边距 24）。此前手工设过 840 / 700，比例都不如框架值 —— 框架这个宽度是按"标题 + 一段话 + 两个按钮"的经典形态调出来的，本弹窗改成窄版分节卡片后同样适配。

### 各块文案

| 块（子标题） | 副标题 |
|---|---|
| 目标程序（手动条目：可更换） | 「程序」页签选磁盘上的文件，「UWP 应用」页签从已安装的应用里挑 |
| 目标程序（系统条目：只读） | 由系统自启动项绑定，不可更改（UWP 来源时：由系统应用清单绑定，不可更改） |
| 延时时间 | 以登录时刻为起点的绝对时间点；同一时间点的多个程序几乎同时启动 |
| 启动身份 | — |

子标题**不带数字序号**（D56）。页签文字：`程序` / `UWP 应用`（默认停在「程序」）。**只读形态下没有页签** —— 系统项的目标由系统绑定，换不了，也就无所谓"哪一类"。

**弹窗标题 / 副标题**

| 场景 | 标题 | 副标题 |
|---|---|---|
| 手动添加 | 手动添加延时启动 | 选择一个程序，由本程序在登录后延时启动 |
| 加入系统项 | 加入延时启动 | 来自「注册表」，加入后原自启动项将被软禁用 |
| 编辑系统项 | 编辑延时设置 | 修改已接管条目，原自启动项保持软禁用 |
| 编辑手动项 | 编辑延时设置 | 手动添加的条目，不关联任何系统自启动项 |

### 字段占位符

> 三项的可见性见上（名称只对手动条目；参数与工作目录除 UWP 外都出现）。

| 字段 | placeholder |
|---|---|
| 显示名称 | 例如：微信 |
| 命令行参数 | 留空则不附加参数 |
| 工作目录 | 留空则使用程序所在目录 |

### 拖放区文案（「程序」页签）

- 空态主文案：`拖入文件，或点击选择程序`
- 空态副文案：`支持 .exe / .lnk / .bat / .cmd / .ps1`
- 选中后：`名称` + `完整路径` + `[更换]`；「更换」重新打开文件选择器。
- 文件类型白名单是硬性的：`.exe` `.lnk` `.bat` `.cmd` `.ps1`。不接受 `.dll`、文件夹、文档。
  - D47 起**移除 `.msi`**：不是 PE 映像，`CreateProcess` 报 `193 ERROR_BAD_EXE_FORMAT`，且安装包不属于"登录后延时启动"的对象。
  - D47 起**新增 `.ps1`**：由 PowerShell 宿主承载（`pwsh.exe` 优先，回落 `powershell.exe`），命令行固定 `-NoProfile -ExecutionPolicy Bypass -File "<脚本>" <参数>`。
  - 白名单的**唯一事实来源**是 `Core.Services.LaunchTargetTypes`，选择器与调度端都读它 —— 界面上这几处文案要跟着它改。
- 拖放（经典 `WM_DROPFILES`）在**两个页签下都生效**，落文件时**自动切回「程序」页签** —— 拖进来的是文件，还停在「UWP 应用」页签的话用户看不见填了什么。
- 🔴 **选完文件不动工作目录框（D58）**：只显示"已选文件"并按需把**显示名称**填成文件名（用户改过名字就不覆盖）；**工作目录保持用户原本的内容**（默认即空）。理由：留空本身有语义（用程序所在目录），自动填一个同样的值只是把"没设置"变成"设置成了同一个值"。

### UWP 应用入口文案（「UWP 应用」页签）

- 入口卡片：`选择 UWP 应用` / 副文案 `从已安装的应用中选择（应用商店应用、系统应用）`
- 选中后卡片替换为：应用显示名 + `UWP 应用 · shell:AppsFolder\<AUMID>` + `[更换]`（重新打开应用列表）
- 选择面板标题：`选择 UWP 应用`；副文案 `只列出开始菜单里能启动的应用。选中后本条目将以普通用户身份、经系统外壳启动。`
- 页签底部一行说明：`UWP 应用由系统外壳激活：始终以普通用户身份启动，命令行参数不会被转发、也没有工作目录可言，因此本页签不提供这两项。`
- 🔴 **页签里没有命令行 / 工作目录两个输入框**（D46 / D48 的结论）：参数不会被转发、UWP 也没有"工作目录"这个概念（打包应用由系统激活器拉起，当前目录由激活机制决定、调用方无从指定；应用也不允许依赖当前目录 —— 数据走 `ApplicationData`、资源走 `ms-appx`）。原先的"置灰 + 一行说明"改为**直接不摆**，理由同上：可编辑却永不生效的字段比没有这个字段更糟。
- 🔴 **启动身份只剩「普通身份」**（D45 真机实测：UWP 进程恒为普通用户身份，即便按管理员身份委托外壳激活也一样）。切到本页签时管理员胶囊被摘掉，切回「程序」页签会**恢复用户之前选的身份**（`_wantAdmin` 单独记一份，不从胶囊反读）。
- 🔴 **只读形态（UWP 来源的系统条目，D54）**：目标程序块只显示应用本身 —— 名称 + `UWP 应用 · shell:AppsFolder\<AUMID>` + 来源行，**名称 / 命令行 / 工作目录一个都不显示**。

### 「移除延时」确认弹窗（分两种文案）

| 条目类型 | 正文 | 主按钮 |
|---|---|---|
| 系统来源 | 该程序的原始自启动项将恢复为**你接管前的状态**。<br>下次登录时它会按系统原本的方式启动，不再受本程序控制。 | 移除并恢复 |
| 手动添加 | 该条目是**手动添加**的，不关联任何系统自启动项。<br>移除后只删除本程序里的这条配置，**系统的任何设置都不会被改动**。 | 移除条目 |

> 手动条目的按钮措辞不能叫「移除并恢复」—— 没有东西可恢复，这个说法会让人以为动了系统。

---

### 页面 3 · 延时启动

**标题**：延时启动
**副标题**：共 5 项（含手动添加 1 项）· 最后一个在登录后 1 分 00 秒启动

**工具栏**：`[🔍 搜索已接管的程序]` … `[立即模拟调度]` `[导出]` `[导入]` `[＋ 手动添加]`

> ⚠️ **工具栏原有的 `[列表 | 时间轴]` 分段控件已按 D37=B 删除**，本页只有列表一种视图。变更说明见本节末。

**列表（按延时分组）**

> **2026-09-20 批复（D51）**：所有条目不再挤成一条平铺列表，改为**按延时值分组**，
> 每组上方一个子标题（子标题 = 延时本身 + 条目数，如 `登录后 30 秒 · 3 项`；延时为 0 的组叫 `立即启动`）。
> 分组的理由是它把"这些条目其实挤在同一秒一起启停"摆到明面上 —— 组内可 `↑` `↓`（FR-4.7 本来就只在组内生效），
> 而跨组的先后由延时值决定，不是拖出来的。表头仍只保留一份，各组共用同一套列宽。
>
> **2026-09-20 批复（D52，版式）**：版式固定为 **子标题 → 边框 → 该组列表**，边框**按组**各一圈
> （此前是整个列表共用一张大卡片、所有组都在里面）。画刷 / 圆角沿用卡片默认值，不新增样式。
> 表头的左内边距 = 组卡片内边距 8 + 行内边距 20 = 28，与行内容逐列对齐。

| 列 | 内容 |
|---|---|
| 顺序 | `1` `2` `3`，同延时内可上下调序 |
| 名称 | 带图标 |
| 命令 | 路径 + 参数 |
| 延时 | `登录后 30 秒`（可点击内联修改） |
| 身份 | `普通用户` / `管理员` |
| 来源 | `注册表 HKCU` / `用户启动文件夹` / `计划任务` / `UWP` / `手动添加` |
| 操作 | `↑` `↓` `编辑` `移除` |

搜索状态下的行为：
- 只影响列表内容，不影响页头统计（统计反映全局）
- `↑` `↓` 置灰，表下补一句：`搜索状态下暂时无法调整顺序。`

**常驻提示条**
> 延时时间是**相对登录时刻**的时间点，不是累加。同一时间点的程序按列表顺序依次发起启动。
> 如果 B 依赖 A 先启动，请给 B 设置**更大**的延时值。

> ⚠️ **本页原有的「时间轴视图」已按 D37=B 删除**（原设计：横轴 `登录` → `10s` → `30s` → `60s` → `1min` → `2min`，节点按延时值定位、同一时点的多个条目纵向堆叠并标注发起顺序，末端标注「开机加载窗口」）。
> 「有依赖关系就给 B 设更大延时」这条语义由上面的常驻提示条承担，不依赖图形表达。**同一决策同时删掉了「总览」页的「开机启动时间轴」卡片**，`TimelineView` 控件不再存在。

**空状态（两种，要分清）**

一条都没有时：
> **还没有延时启动项**
> 到「自启动项」页找到想延后的程序，点击「延时启动」；或者直接手动添加一个程序。
> [前往自启动项] [手动添加]

有数据但搜索无结果时：
> **没有匹配的延时条目**
> 换个关键词试试，或清空搜索框。
> [清空搜索]

**「移除延时」确认弹窗** → 见三之二节末，按条目类型分两种文案。

---

### 页面 4 · 系统启动项（只读）

**标题**：系统启动项
**副标题**：服务、驱动、登录组件的只读检查

**顶部常驻警示条**
> 🛡 **此页仅供查看，不参与延时启动**
> 系统服务、内核驱动、Winlogon 组件的启动顺序由 Windows 控制。强行延时会拖慢或破坏系统启动，这里只做展示与安全排查。

**分组与文案**

**① 系统服务（自动启动）**
> 服务的正确延时方式是改用 Windows 原生的「自动（延迟启动）」——系统启动约 2 分钟后拉起，比本程序接管更安全。
> 列：服务名 / 显示名 / 启动类型 / 操作
> 操作：`[改为延迟自动启动]`（仅对「自动」类型显示）、`[恢复为自动]`（仅对「延迟自动」显示）

**② 内核驱动**
> ⚠ 驱动启动顺序由内核即插即用管理器控制，不可延时。此项为只读。
> 列：驱动名 / 显示名 / 启动类型 / 状态

**③ 登录组件（Winlogon / 登录脚本）**
> ⚠ 这些位置一旦改动错误，可能导致**无法登录**。同时它们也是恶意软件常见的藏身处 —— 请核对每一项是否为你认识的程序。
> 列：位置 / 内容 / 风险
> 风险徽标：`系统默认` / `第三方` / `⚠ 可疑`

**④ 已计划的其他自启位置**（可选，🟢C 优先级）
`RunOnce` / `RunServices` / `ShellServiceObjectDelayLoad` 等 —— 仅展示。

---

### 页面 5 · 运行日志

**标题**：运行日志
**副标题**：调度器每次开机的执行记录
**工具栏**：`[日期 ▾ 今天]` `[级别 ▾ 全部]` `[打开日志文件]` `[清空日志]`
**表格**：时间 / 级别 / 条目 / 结果 / 详情
**空状态**：`暂无日志` / `重启电脑后，调度器的执行记录会出现在这里`

---

### 页面 6 · 设置

**顶部固定说明（非设置项）**
> 以下行为是**固定设计**，不提供开关：进入程序时**自动扫描一次**自启动项；关闭窗口**即退出程序**（不常驻托盘，开机调度由计划任务负责）。

> **2026-09-20 批复（D50）**：设置页实现后的分区顺序是 **外观 → 延时 → 调度**（「外观」提到最前，作为第一个子标题）。
> 本节下面的表格是原始分组计划；其中"启动方式 / 数据 / 关于"三组按 UI v2.2 批复已分别并入或删除
> （降权与托盘设置语义固定、不再可配），**延时**组的默认预设值见下表（已按 D50 更新）。

**分组：延时启动**
决定弹窗里可选的延时值，以及调度器的存活时长。

| 设置项 | 类型 | 默认 | 说明 |
|---|---|---|---|
| 预设延时选项 | 胶囊列表（可增删） | `0, 5, 10, 15, 20, 30, 60` | 「延时启动」弹窗里的快捷按钮。用户可改成自己的习惯值（如 `0,15,45,90`） |
| **默认预设（胶囊选中态）** | **胶囊单选** | **`10 秒`**（D63 起） | 被选中的那一颗就是**默认值** —— 接管条目时编辑器预选它。必须是预设列表的成员，加载时兜底到第一项。⚠️ 已落盘的 `config.json` 原样保留，改默认值只影响新装 / 未改过该项的配置 |
| ~~单条目延时上限~~ | — | — | **已删除**（2026-09-19 批复：用配置去限制调度能力是反的 —— 用户想设 1 小时延时被卡在 10 分钟，那不是"延时启动"而是"启动失败"） |
| **预计调度器驻留时长** | **只读计算值** | 约 1 分 10 秒 | = 最长延时 + 托盘保留 + 启动开销。**自动算出来，不是设置项** |

**分组：调度端**（登录后执行的那个轻量进程，详见 `scheduler-design.md`）

| 设置项 | 类型 | 默认 | 说明 |
|---|---|---|---|
| 显示托盘进度图标 | 开关 | `开` | 关掉后调度端完全无形（"隐形版"），只有完成通知 |
| 完成时通知 | 下拉 | `仅失败时` | 仅失败时 / 总是 / 从不。全部成功是预期结果，默认不打扰 |
| 启动失败时重试 | 下拉 | `1 次` | 不重试 / 1 次 / 2 次 |
| 连续失败后交还自启动 | 下拉 | `3 次` | 关 / 3 次 / 5 次。避免某程序被永久变成"永不启动" |
| 完成后托盘图标保留 | 下拉 | `3 秒` | 3 秒 / 5 秒 / 10 秒。仅图标消失时机，不是面板 |

**分组：启动方式**
- 普通用户程序优先降权启动（CreateProcessAsUser）— `开`
  └ 说明：避免被本程序的管理员权限影响，导致拖拽、剪贴板、文件权限异常
- 降权失败时回退为直接启动 — `开`

**分组：数据**
- 配置目录 — `C:\Users\...\AppData\Local\DelayStart` `[打开目录]`
- `[导出配置]` `[导入配置]` `[重置所有设置]`

**分组：关于**
- 版本 / 构建时间 / 调度器版本 / `[查看许可]`

---

## 四、调度端（登录后执行的轻量进程）界面

> **完整设计已拆分为独立文档：`docs/scheduler-design.md`。**

**原方案已推翻**：早期设计是在屏幕右下角放一个常驻进度面板。**这个设计是错的** —— 调度器的存活时长等于最长延时（用户可设数十分钟），而这恰好是用户刚登录、正在干活的时段。登录后抢屏的窗口，无论做得多精致都是错的。

**新方案（一句话）**：调度端**没有主窗口**，只有一个托盘图标。默认完全隐形；用户悬停看一行摘要、点击弹出面板（失焦即关）；调度结束后发系统通知，**默认只在有失败时发**；点击通知打开管理端「运行日志」页并定位本次运行。

信息通道分 5 级，默认停在 0 级（静默），任何升级都必须由用户主动触发（失败提醒除外）：

| 级别 | 通道 | 用户动作 |
|---|---|---|
| 0 | 完全静默 | — |
| 1 | 托盘图标 + 悬停 tooltip | 主动悬停 |
| 2 | 托盘弹出面板 | 主动点击，失焦即关 |
| 3 | 系统通知（默认仅失败） | 系统弹出，可点击 |
| 4 | 管理端「运行日志」页 | 主动打开完整程序 |

**两条硬技术约束**（详见 `scheduler-design.md` 第五节）：

1. 调度器是管理员权限进程，Windows 通知平台**不支持 elevated 进程发真 Toast**；NativeAOT 也用不了 WinRT / Windows App SDK 的通知 API。只能用托盘气泡通知（`Shell_NotifyIcon`）。
2. Win11 上气泡通知**不进通知中心**（Win10 会留，Win11 变瞬时）。所以通知**不能是唯一通道**，必须有兜底 —— 管理端总览页的「上次运行结果」横幅 + 完整运行日志。

---

## 五、视觉规范

| 项 | 规范 |
|---|---|
| 窗口 | 无系统标题栏，`ExtendsContentIntoTitleBar`，Mica 背景 |
| 窗口默认尺寸 | **1400×900 DIP**（内容区），首次打开把 1000 宽的卡片居中、左右留白对称；按显示器 DPI 换算成像素，最多占工作区 92%，并相对所在显示器居中（D49） |
| 窗口最小尺寸 | **960×600 DIP**：再窄延时启动页的列会互相挤、再矮列表只剩一两行（D49。原生 WinUI 没有最小尺寸 API，走 `WM_GETMINMAXINFO`） |
| 主题 | 跟随系统明暗；设置页可强制浅色/深色 |
| 强调色 | 跟随系统 Accent（默认 Win11 蓝） |
| 圆角 | 卡片 8px，控件 4px，窗口 8px |
| 字体 | 标题 `Segoe UI Variable Display Semibold`，正文 `Segoe UI Variable Text`（中文回退 `Microsoft YaHei UI`） |
| 字号 | 页面标题 28 / 卡片标题 14 半粗 / 正文 14 / 辅助文字 12 |
| 图标 | `Segoe Fluent Icons`；列表行使用真实程序图标（`IShellItemImageFactory`，按 D30 与 `architecture.md` 3.4；`SHGetFileInfo` 只能给 16/32px 高 DPI 会糊，已否决） |
| 间距 | 页面边距 32；卡片内边距 16；列表行高 64（两行式） |
| 状态色 | 已启用 `#8A8A8A` / 已禁用 `#FCE100`（暗）`#9D5D00`（亮） / 已接管 = Accent / 已失效 `#FF99A4` / 受保护 灰+锁 |

---

## 六、决策点与批复结果

> **状态图例**：`✅` 已批复并锁定 · `⏳` 尚未批复（列出建议值，**未批复前不进入编码**）
> **D1–D25 已于 2026-09-19 全部批复完毕，决策冻结。D27 / D28 / D29 于 Phase 1 期间批复（均为 Phase 1 暴露的分层与 AOT 边界问题）。仅 D26 待决策（测试命令走向），不阻塞 Phase 1。**
>
> - D1–D3、D14–D19：首轮批复
> - D4–D13：2026-09-19 按建议值批量批复
> - D20：**改判为全程提权**（原「按需提权」建议被否，理由见 `requirements.md` NFR-3 注）
> - D21：xUnit v3
> - D22：**分发形态改为 Inno Setup 安装器 + unpackaged**，D8 的「第一版 zip」被取代（理由见 6.4）
> - D23：**安装与数据布局**改为 per-user 不提权安装 + 配置走 Roaming、日志走 Local（理由见 6.5）
> - D24：**调度端 UI 改纯 Win32**（Phase 0 发现 WinForms + NativeAOT 是官方不支持的组合，**重开 D1**；理由见 6.6）
> - D25：**Phase 0 模板页全部删除**，导航壳刻意留空，Phase 3 从零建 6 个页面（理由见 6.7）
> - D26：**`dotnet test` 在本项目不可用**，测试命令改用 `dotnet run`；是否补 VSTest 适配器待定（理由见 6.8）
> - D27：**Phase 1 只做 Core 层接口与纯逻辑**，P/Invoke 启动器推到 Phase 4（理由见 6.9）
> - D28：**UWP 激活改用 `explorer.exe shell:AppsFolder\<AUMID>`**，弃用 COM（理由见 6.10，新增 R12）
> - D29：**Phase 1 期间的 4 项文档修订**（F1–F4），不改变任何行为约定

| 编号 | 决策 | 选项 | 批复 | 优先级 |
|---|---|---|---|---|
| **D1** | 调度端技术 | A. 独立轻量进程 WinForms+AOT（≈6MB，冷启动 <0.3s）<br>B. WinUI 3 调度端（体积大、冷启动 1–2s） | **✅ A** ⚠️ **被 D24 重开**（2026-09-19 Phase 0 发现 WinForms + AOT 是官方不支持的组合）。**注**：原表此处写"WinUI 3 的 AOT 仍 preview"——该说法已过期，WinAppSDK 1.6 起官方支持 AOT，但冷启动与体积两条理由仍成立，见 6.6 | 🔴 |
| **D2** | 主窗口导航 | A. 左侧 NavigationView + 来源子项<br>B. 顶部 TabView（沿用 demo 结构）<br>C. 单页 + 筛选 Chips | **✅ A** | 🔴 |
| **D3** | 界面语言 | A. 仅简体中文<br>B. 中英双语（资源文件） | **✅ A** | 🟡 |
| **D4** | UWP 延时语义 | A. 不做「延时启动 UWP」，UWP 项只能禁用/启用<br>B. 做「禁用系统自启 + 到点 COM 激活」 | **✅ B** —— UI 文案写明"UWP 延时启动会绕过系统的启动管理" | 🟡 |
| **D5** | 「系统启动项」只读页 | A. 做（含服务 delayed-auto 一键切换）<br>B. 只做服务 delayed-auto 切换，其余不做<br>C. 整页砍掉，后置 | **✅ B** —— 服务用系统原生 `delayed-auto`；驱动/Winlogon 不做 | 🟡 |
| **D6** | 程序名 | A. 延时启动管理器 / DelayStart<br>B. 你给一个 | **✅ A**（**⚠️ 已被 D61 改判**：应用名只用英文 `DelayStart`，中文名退回文档） | 🟢 |
| **D7** | 「立即模拟调度」 | A. 做（真的按延时启动程序，0.5 倍速预览**进度视图**）<br>B. 只做动画，不真启动<br>C. 不做 | **✅ A**（原写"按时间轴…预览"；**时间轴已按 D37=B 整体删除**，改为进度视图） | 🟢 |
| **D8** | 分发方式 | A. 免安装 zip（unpackaged，自包含）<br>B. MSIX 安装包<br>C. Inno Setup 安装器 | ~~**✅ A → C**~~ → **被 D22 取代：直接走 C** —— 第一版即出安装器 | 🟢 |
| **D9** | `.lnk` 图标与目标解析 | A. 做（列表更易读）<br>B. 不做，直接显示 .lnk 路径 | **✅ A** | 🟡 |
| **D10** | 批量操作 | A. 做（多选 + 批量禁用/批量延时）<br>B. 不做，第一版先单项 | **✅ B** —— 先跑通单项链路 | 🟢 |
| **D11** | 预设延时选项是否可配置 | A. 只在代码里写死 5 个值<br>B. 设置页可编辑预设值 | **✅ B** —— 逗号分隔文本框驱动弹窗按钮 | 🟡 |
| **D12** | 延时输入上限的语义 | A. 独立的"最大延时上限"限制调度<br>B. 改名「单条目延时上限」，**只做输入校验**，不影响调度 | **✅ B** | 🔴 |
| **D13** | 长延时条目（> 10 分钟）的调度方式 | A. 统一由本程序调度器驻留等待<br>B. 长延时改用 Windows 计划任务 `onlogon + delay`<br>C. 设置里给开关，由用户选 | **✅ A** —— 机制/面板/日志三者统一；驻留代价可忽略 | 🟡 |
| **D14** | 调度端整体形态 | A. **无主窗口**，仅托盘图标 + 点击弹出面板<br>B. 屏幕右下角常驻进度面板（原设计）<br>C. 完全无形，只发通知 | **✅ A** | 🔴 |
| **D15** | 完成后通知策略 | A. 仅失败时通知<br>B. 总是通知<br>C. 从不通知 | **✅ A** | 🔴 |
| **D16** | 面板里是否提供 `[退出调度]` | A. 不提供（急用程序直接双击程序）<br>B. 提供，退出时恢复未启动项的自启动 | **✅ A** | 🟡 |
| **D17** | 启动失败的长期策略 | A. 永久保持接管、静默重试<br>B. 一次失败即交还自启动<br>C. 连续 3 次失败后**自动**交还自启动<br>D. **不自动处理**：保持接管 + 持续升级提醒，最终由用户手动处理 | **✅ D** | 🟡 |
| **D18** | 点击完成通知的行为 | A. 打开管理端「运行日志」页并定位本次运行<br>B. 只弹出托盘面板 | **✅ A** | 🟢 |
| **D19** | 管理端显示"调度进行中"的实时状态 | A. 做（读 `state\current-run.json`）<br>B. 不做，管理端只显示配置 | **✅ A** | 🟢 |
| **D20** | 管理端权限模型 | A. **全程**提权（`app.manifest` → `requireAdministrator`）<br>B. 按需提权（默认普通权限，遇 HKLM 时提示重启为管理员） | **✅ A** —— 原建议 B 被否。**核心功能 100% 需要管理员**，按需提权只省掉"查看"这一步的 UAC，却引入提权重启 + 意图传递 + 半完成状态处理，不划算。详见 6.2 | 🔴 |
| **D21** | 单元测试框架 | A. xUnit v3（`xunit.v3`）<br>B. xUnit v2（SDK 内置模板）<br>C. NUnit / MSTest | **✅ A** —— 主选 xUnit v3；v3 模板安装受阻时降级 B。**NUnit 明确排除**（.NET 10 SDK 下存在 MTP 版本冲突 CS1705）。详见 6.3 | 🟡 |
| **D22** | 分发形态：MSIX 是否可行 | A. Inno Setup 安装器 + **unpackaged**<br>B. MSIX 打包 + 自写提权 bootstrapper<br>C. 维持 D8 的第一版免安装 zip | **✅ A** —— MSIX 三条致命理由（Win10 无法提权 / 注册表虚拟化 / 安装路径含版本号），详见 6.4。同时取代 D8 | 🔴 |
| **D23** | 安装与数据布局 | A. 装 `Program Files`（需提权安装）+ 数据全放 `%LOCALAPPDATA%`<br>B. **per-user 装 `%LOCALAPPDATA%\Programs\DelayStart`（不提权）**；配置 `%APPDATA%\DelayStart`（Roaming）；日志/状态 `%LOCALAPPDATA%\DelayStart`（Local） | **✅ B** —— 用户定：程序不放 Program Files、配置走 Roaming、日志走 Local。派生三条：安装器 `PrivilegesRequired=lowest`、计划任务改由管理端首次启动注册、卸载默认保留配置与日志。详见 6.5 | 🔴 |
| **D24** | 调度端 UI 技术选型（Phase 0 重开 D1） | A. 维持 **WinForms + AOT**（靠内部属性放行，**官方不支持**）<br>B. 改 **纯 Win32**（`Shell_NotifyIcon` + 自绘弹窗，**官方完全支持**）<br>C. 改 **WinUI 3 + AOT**（官方支持，但冷启动 0.5–0.7s 超 NFR-1.2，且 WinUI 3 **无托盘 API**）<br>D. **放弃 AOT**（冷启动 1–2s + 需装 Desktop Runtime） | **✅ B** —— 用户批复。Phase 0 已按 B 落地骨架：调度端 csproj 去掉 `UseWindowsForms` 与 `_SuppressWinFormsTrimError`，`Program.cs` 改为纯 Win32 入口。详见 6.6 | 🔴 |
| **D25** | Phase 0 模板残留页（`Pages/` 的 Home / About / Settings） | A. **现在就删**，导航壳留空，Phase 3 从零建 6 个页面<br>B. 留到 Phase 3 整体替换 | **✅ A** —— 模板页与 `architecture.md` 1.4 的页集不符，留着只是 6 个死文件 + 假导航。已执行。详见 6.7 | 🟢 |
| **D26** | 测试命令：`dotnet test` 集成有缺陷，怎么走 | A. **只改命令**（用 `dotnet run`，零改动）<br>B. 补 `xunit.runner.visualstudio` + 退回 VSTest 路径（VS 测试资源管理器可用）<br>C. 换掉 `xunit.v3.mtp-v2` 改用 `xunit.v3` | ⏳ **待决策** —— AI 建议 **A（已生效）**。`xunit.v3.mtp-v2` 4.0.1 + SDK 10.0.401 下 `dotnet test` 报零测试（发现本身正常）；B/C 属优化，**不阻塞 Phase 1**。详见 6.8 | 🟡 |
| **D27** | Phase 1（Core 层）是否连同 P/Invoke 启动器一起写 | A. **本期只做接口 + 纯逻辑**，`ProcessLauncher` / `TokenHelper` / `Core/Interop/*` 推到 Phase 4<br>B. 本期连 P/Invoke 一起写完（照 demo 移植） | **✅ A** —— 用户批复。P/Invoke 启动器的正确性只能在**真实登录会话**里验证（见 6.9），Phase 1 写了只能靠编译通过自证。派生出 `IProcessLauncher` 接口留白 + 已落地的判定逻辑 | 🔴 |
| **D28** | UWP 项怎么激活（AOT 约束下） | A. **`explorer.exe shell:AppsFolder\<AUMID>`**（纯 `Process.Start`，零 COM）<br>B. 维持 demo 的 `IApplicationActivationManager` + `Marshal.GetObjectForIUnknown`<br>C. 砍掉 UWP 延时（回退 D4 的 A） | **✅ A** —— 用户批复。B 在 NativeAOT 下运行时必抛 `PlatformNotSupportedException`（R12），与 D24=B 的 AOT 前提直接冲突；C 推翻 D4。代价：无 PID → 机制 7 不适用，`null` 快照判成功。详见 6.10 | 🔴 |
| **D29** | Phase 1 期间的文档修订（F1–F4） | F1 `requirements.md` 追踪矩阵实现层 6 行路径<br>F2 `architecture.md` 2.1 / 2.2 补 `PathService` 等<br>F3 删 Core 的 `Interop/Shell32.cs`<br>F4 登记 R12 | **✅ 全部执行** —— 均为一句话级修正，不改变行为约定，故并入 Phase 1。详见 `changelog.md` 第七轮 | 🟢 |
| **D30** | Phase 2 是否连 `IconProvider` / `ServiceQueryService` / `SystemStartupInspector` 一起做 | A. **不做**，推到 Phase 3（图标）/ Phase 5（系统启动项只读页）<br>B. 本期一并写完 | **✅ A** —— 三者本期没有消费者，且进不了单元测试（14.1），写了只能靠"编译通过"自证 = 假绿灯，与 D27 同一条逻辑。详见 6.11 | 🟡 |
| **D31** | `FailureStreakService`（连续失败聚合）放 Core 还是 Management | A. **搬进 Core**，两端共用<br>B. 留在 Management | **✅ A** —— 原文档三处互相矛盾（调度端要角标 / 调度端只引用 Core / 却写"由管理端聚合"），该链路原本实现不了。计算是纯函数，AOT 安全。**属文档错误修正**。详见 6.11 | 🔴 |
| **D32** | Phase 2 没有 UI，安全验收怎么执行 | A. **给管理端加 headless CLI**（`--restore-all` / `--reinstall-task` / `--takeover` / `--release`）<br>B. 等到 Phase 3 有 UI 再验 | **✅ A** —— B 会让整个 Phase 2 的产物无法被真实执行。`--restore-all` 本就是 D22 卸载可逆性的必需品。详见 6.11 | 🔴 |
| **D33** | Management 层单元测试范围 | A. **本期做**（假 `IStartupSource` 测 FR-1.4、假 Store 测 FR-3.1 回滚）<br>B. 只做手工验证 | **✅ A** —— 编排逻辑用假实现可确定性覆盖，含手工几乎测不出的"第 3 步抛异常时前两步是否被撤销"。详见 6.11 | 🟡 |
| **D34** | `DelayStartScheduler` 本期真机注册还是推 Phase 4 | A. **本期做并验证**<br>B. 推到 Phase 4 | **✅ A** —— 验证不需要调度端能跑，`schtasks /query /v /fo LIST` 就能读出三要素与身份。B 会把最易静默出错（SYSTEM 身份下配置读不到且不报错）的环节拖到最后。详见 6.11 | 🟡 |

> **D14–D19 归属调度端**，完整论证、技术硬约束（AOT / elevated 通知限制）、通知文案矩阵、跨进程状态文件设计见独立文档 **`docs/scheduler-design.md`**。
> **D20 / D21 / D22 / D23 / D24 的完整论证见下方 6.2 / 6.3 / 6.4 / 6.5 / 6.6**，工程落地见 `architecture.md` 1.4 / 1.5 与 `build-and-test.md` 第一、二、七节。

### 6.1 D17 批复说明：失败由用户处理，软件不代劳

**批复：不自动处理。** 原建议 C（连续 3 次失败后自动交还自启动）被否，理由充分且比原方案更正确：

1. **自动交还自启动 = 软件在用户不知情时改了系统状态**，直接违反本项目"全部可逆、且由用户掌控"的核心原则。
2. **失败原因多数不归本软件管**（程序被卸载、被杀软拦截、路径变更）。自动恢复自启动只是把问题从"不启动"变成"启动失败弹错误框"，还顺手把用户设的延时取消了 —— 问题没解决，配置先丢了。
3. **静默改配置比明示失败更糟**。用户宁可看到一条一直存在的警告，也不愿某天发现"我明明设了延时，怎么没生效"。

**改为三条约束：**

| 约束 | 内容 |
|---|---|
| 保持接管 | 失败项**始终**保持软禁用 + 接管状态，下次登录按原延时继续尝试。软件永不自动修改该项的系统状态 |
| 提醒必须持续 | 失败提示不能只出现一次。托盘角标保留、管理端横幅常驻，直到用户显式处理（见 `scheduler-design.md` 第八节） |
| 出口只有一个 | 管理端横幅上提供 `[移出延时启动]` 直接跳转到该条目的移除确认流程（系统条目走"移除并恢复"），由用户点击完成 —— **软件只给路径，不做决定** |

### 6.2 D20 批复说明：改为全程提权（原「按需提权」建议被否）

**批复：全程 `requireAdministrator`。** 我原来提的"按需提权"是错的。致命的判断错误在于：我按"功能数量占比"评估，而正确的评估口径是**"有没有任何一个功能必须提权"**——只要有一个，按需提权就省不掉那次 UAC，剩下的只是复杂度。

**必须管理员权限的操作：**

| 功能 | 涉及位置 | 不提权的后果 |
|---|---|---|
| **注册调度器计划任务** | Task Scheduler，且需 `RunLevel=Highest` | **软件根本不能工作** |
| 软禁用 HKLM Run 项 | 写 `HKLM\...\StartupApproved\Run` | 拒绝访问 |
| 软禁用 HKLM 32 位 Run 项 | 写 `HKLM\...\StartupApproved\Run32` | 拒绝访问 |
| 软禁用系统启动文件夹项 | 写 `HKLM\...\StartupApproved\StartupFolder` | 拒绝访问 |
| 禁用系统级 / 他人计划任务 | `task.Enabled = false` | 拒绝访问 |
| 服务切 `delayed-auto`（D5） | `ChangeServiceConfig` | 拒绝访问 |

**不需要管理员权限的操作：**

| 功能 | 位置 |
|---|---|
| 扫描 + 软禁用 HKCU Run | `HKCU\...\StartupApproved\Run` |
| 扫描 + 软禁用用户启动文件夹项 | `HKCU\...\StartupApproved\StartupFolder` |
| 只读扫描 HKLM / 计划任务 / 服务 | 默认 ACL 允许读 |
| UWP 自启状态读写 | `HKCU\...\AppModel\SystemAppData` |
| 配置与日志 | 配置 `%APPDATA%\DelayStart`（Roaming）· 日志 `%LOCALAPPDATA%\DelayStart\logs`（Local） |

**决定性理由：**

1. **核心链路 100% 阻塞**。不注册调度器，软件连"延时启动"这个主功能都不存在。既然无论如何都要提权，"按需"就只省了"只看不改"这一种用法。
2. **按需提权会制造半完成状态**。用户连点 10 个禁用，HKCU 的 6 个成功、HKLM 的 4 个弹 UAC 或失败——系统进入一个**用户无法理解的中间状态**："为什么这几个禁用了那几个没有？"这比"一开始就不让用"糟得多。
3. **按需提权的实现成本吃掉一个 Phase**：需要 `runas` 重启进程 + 命令行传递待执行意图（哪个页、哪个条目、什么操作）+ 重启后恢复 UI 状态 + 处理用户拒绝 UAC 时的回滚。这些代码全部是纯负担，不产生任何用户价值。
4. **参考同类软件**：Autoruns（微软官方）、Process Explorer、`regedit`、`services.msc`、任务计划程序——**全部是打开即提权**。用户的预期已经建立：系统配置工具就该弹 UAC。
5. **使用频率极低**：配置一次就不再打开。一年弹几次 UAC 完全可接受。
6. **与调度端一致**：调度端本身就是 elevated 进程（D1，为了免 UAC 启动管理员程序）。两端权限一致，语义统一，不需要"我这次启动是什么权限"的心智模型。

**代价与必须配套的三条约束：**

| # | 问题 | 对策 |
|---|---|---|
| 1 | **UAC 被拒后不能降级运行** | 明确提示 `本程序需要管理员权限才能管理自启动项。` + `[重试]` `[退出]`，然后**退出**。半残运行会造成"用户以为禁用成功、实际静默失败"的最坏情况 |
| 2 | **提权窗口接收不到资源管理器的拖放（UIPI）** | 见 `architecture.md` R10。主路径是 `[浏览…]` 按钮（100% 可用）；拖放作为增强，用 `ChangeWindowMessageFilterEx` + 旧式 `WM_DROPFILES` 尝试补上，不通则拖放区降级为纯按钮 |
| 3 | **提权状态下"拖出"到其他应用不工作** | 无解（UIPI 强制）。**不接受"拖 exe 到任务栏固定"这类交互设计** |

**明确不支持：**`以其他用户身份运行`，**以及 OTS（over-the-shoulder）提权**（在 UAC 里输入另一个管理员账户的凭据）。这两种情况都会让 `%APPDATA%` / `%LOCALAPPDATA%` 指向**另一个账户**，配置读不到、日志写错位置。

判定方式（唯一正确做法，NFR-3.8 / E18）：比较**提权后的用户 SID** 与**交互会话用户 SID**（取本会话 `explorer.exe` 的令牌用户）。`IsInRole(Administrator)` 只回答"是否提权"，回答不了"是不是换了人"—— 用错了会把正常的 UAC 自提权误判为不支持。需在文档与验收清单中写死。

**新增的唯一界面元素：UAC 被拒提示**（这是 D20 改判带来的全部 UI 增量）

```
┌──────────────────────────────────────────────┐
│  ⚠  需要管理员权限                           │
│                                              │
│  DelayStart 需要管理员权限，才能管理         │
│  HKLM 自启动项、系统启动文件夹与开机调度      │
│  任务。                                       │
│                                              │
│  可以重新启动并同意 UAC 提示，或选择退出。    │
│                                              │
│                    [重试]      [退出]        │
└──────────────────────────────────────────────┘
```

- 在 `App.OnLaunched` **最早期**判断权限，**不构建主窗口**——避免用户看到一个残缺的界面
- `[重试]`：以 `runas` 重新启动当前 exe
- `[退出]`：直接退出
- 文案不出现"部分功能可用"之类的暗示——**不存在半可用状态**（`NFR-3.6`）

### 6.3 D21 批复说明：xUnit v3

**先说清一个事实**：微软对 WinUI 3 测试**没有单一推荐框架**——官方文档（*Test apps built with the Windows App SDK and WinUI 3*）把 MSTest / NUnit / xUnit **三者并列**，内容只讲"引用 WinUI 3 项目时要改 TargetFramework、加 `WindowsAppSDKSelfContained`"。所以选 xUnit 不是"照官方推荐"，而是基于下列技术理由：

| 框架 | 结论 | 理由 |
|---|---|---|
| **xUnit v3** | ✅ **选它** | 单一 NuGet 包（`xunit.v3.mtp-v2`）、原生 MTP、`OutputType=Exe` 可独立运行、`Assert.Skip` 支持条件跳过、无版本冲突。v3 要求 .NET 8+，.NET 10 满足 |
| xUnit v2 | 🟡 降级备选 | SDK 内置模板 `dotnet new xunit` 直接可用（无需额外装模板），但要拖 `xunit.runner.visualstudio` + `Microsoft.NET.Test.Sdk` 走 VSTest 老路径。**若 v3 模板安装受阻就用它**，测试代码本身零改动（`[Fact]`/`[Theory]`/`Assert` 完全一致） |
| NUnit | ❌ 排除 | .NET 10 SDK 下有已知缺陷：`NUnit3TestAdapter` 5.1.0 与 SDK 注入的 `Microsoft.Testing.Platform` 版本冲突，报 **CS1705 且不给出可诊断的包依赖**，必须手动 pin 两个平台包才能编译。**不给项目埋这种雷** |
| MSTest | ❌ 排除 | 其独有价值是 `[UITestMethod]`（XAML 线程测试），而**第一版不做 UI 自动化测试**，用不上；语法也更啰嗦 |

**关键结构性收益**：因为 `DelayStart.App`（WinUI 3 项目）**不被任何测试项目引用**，测试项目就完全不需要走微软那套"WinUI 3 测试项目配置"。TFM 只需跟到 `net10.0-windows`（与 `Core`/`Management` 一致），既不需要 `net10.0-windows10.0.19041.0`，也不需要 `WindowsAppSDKSelfContained`，更不需要预装 Windows App Runtime。测试跑起来是秒级的。

> 这是 1.1 节"按 AOT 兼容性分层"的额外红利：UI 与逻辑彻底分离后，逻辑测试也就不必背 WindowsAppSDK 的包袱。

**主次两条命令路径**（详见 `build-and-test.md` 4.1）：

```bash
# 主路径：先装 v3 模板（一次性）
dotnet new install xunit.v3.templates
dotnet new xunit3 -o tests/DelayStart.Core.Tests -f net10.0
#   随后把 csproj 里 <TargetFramework> 改为 net10.0-windows，与 Core / Management 对齐

# 降级路径：若模板安装失败，用 SDK 内置模板
dotnet new xunit -o tests/DelayStart.Core.Tests -f net10.0
```

### 6.4 D22 批复说明：分发形态定为 Inno Setup 安装器 + unpackaged（MSIX 被否决）

**批复：A。** 同时**取代 D8** —— 不再有"第一版免安装 zip"这个阶段，第一版就出安装器。

提前说明：我上一轮给的理由（"MSIX 有致命缺陷"）方向对，但没给出**最硬的那一条**。下面是补齐后的完整否决链，按力度排序。

#### 否决 MSIX 的三条理由

| # | 理由 | 依据 | 后果 |
|---|---|---|---|
| 1 | 🔴 **打包应用提权只在 Windows 11 及以上可用** | WindowsAppSDK issue **#896** 官方 feature status 原文：<br>*"Supporting elevated packaged apps using framework package APIs (like WinAppSDK and WinUI 3) required OS-level changes. Thus, this fix is only available in Windows 11 and higher."* | 与 `NFR-5.1`（Win10 21H2 及以上）**直接冲突** → **D20 在 Win10 上无法实现**。这一条单独就足以否决，不需要看后两条 |
| 2 | 🔴 **注册表虚拟化会吞掉 HKCU 软禁用写入** | MS Learn *Flexible virtualization*：打包应用的 HKCU **运行时写入**进入"每个应用、每个用户独立的私有结构"，**只有该应用自己看得见** | 软禁用的 HKCU 部分（HKCU `StartupApproved\Run`、`StartupFolder`、UWP `State`）**静默失效**：我们以为写成功了，任务管理器、资源管理器、**我们自己的调度端都看不到标记**。而"静默失效"正是本项目明令禁止的失败模式（`坑 1` / `坑 5` 是同一个病根） |
| 3 | 🔴 **安装路径含版本号** | 打包应用的安装位置为 `C:\Program Files\WindowsApps\<Name>_<Version>_<Arch>__<PublisherId>` | 每次更新路径都变 → 计划任务 action 指向的路径失效 → **调度链路断**。这正是本轮要解决的问题，不能引进新版本 |

#### 理由 2 的"关掉虚拟化"这条路也堵死

关闭注册表/文件虚拟化需要两个**受限能力**（restricted capability）：

```xml
<rescap:Capability Name="unvirtualizedResources" />   <!-- 关虚拟化 -->
<rescap:Capability Name="allowElevation" />           <!-- 配合 exe manifest 提权 -->
```

连带后果：

| 后果 | 说明 |
|---|---|
| **App Installer 装不了** | 带受限能力的包双击 .msix 报 `Cannot open app package`；只能**提权 PowerShell `Add-AppxPackage`**，或自写一个提权 bootstrapper exe 代装。分发体验**反而比 Inno Setup 差** —— 用户要经历"先右击 PowerShell 以管理员运行，再敲命令" |
| **商店上架基本不可能** | 官方关于 `allowElevation` 的原文：*"In most cases, the use of this capability won't be approved."* |
| 旁加载无需审批 | 这是唯一的利好，但**抵消不了上面两条**，而且抵消不了理由 1 |

**结论：MSIX 不是"稍微麻烦一点"，而是"在 Win10 上根本做不到 D20"。** 选项 B（MSIX + 自写提权 bootstrapper）本质上是"用打包格式 + 一个安装器"，那我们为什么不直接用原生安装器？

#### D22 选项 A 的具体内容

| 项 | 内容 |
|---|---|
| 应用形态 | `WindowsPackageType=None`（unpackaged）+ `app.manifest` 的 `requireAdministrator`。**真实注册表、真实文件系统，无任何虚拟化** |
| 安装目录 | **固定路径** `{localappdata}\Programs\DelayStart`（**per-user，不放 Program Files**），**不含版本号** → 计划任务 action 路径永不失效。路径与数据布局的完整规范见 **D23** / `architecture.md` 1.5 |
| 安装器 | Inno Setup（**本机已装 6.7.3**，`winget list -q innosetup` 实测），脚本入库 `installer/DelayStart.iss` |
| 安装器职责 | 拷贝文件 → 开始菜单快捷方式 → 注册卸载信息。🔴 **不注册计划任务**（D23：注册必须提权，改由**管理端首次启动时幂等注册**，全流程 UAC 只弹那一次） |
| 卸载器职责 | 🔴 **先还原全部被接管的条目与软禁用标记，再删文件**。🔴 **实现位置必须是 `[Code]` 的 `InitializeUninstall()`，不能用 `[UninstallRun]`** —— 后者读不到退出码，做不到"失败即中止"。见下方硬约束与 `build-and-test.md` 7.2 |
| 体积 / 运行时依赖 | **由 Phase 0 的 R9 结果决定**：R9 通过 → `WindowsAppSDKSelfContained=true`（约 100–150 MB，零依赖）；R9 不通过 → 走 R9 降级路径①框架依赖（约 10–15 MB，**安装器负责部署 Windows App Runtime**）。<br>**这是 D22 与 R9 降级路径的天然结合点** —— 有了安装器，"让用户自己装运行时"这个代价就消失了，R9 不通过也不再是问题 |

#### 新增硬约束（🔴 优先级高于任何视觉与体积优化）

| 约束 | 内容 | 违反的后果 |
|---|---|---|
| **卸载必须可逆** | 用户卸载时，所有被接管的条目**必须还原到接管前的自启动状态**（延时配置可以丢弃，**自启动状态必须还原**） | 用户卸载后所有程序永久不再自启，而他根本不知道原因。**这会是本项目最严重的缺陷** |
| **禁止"先删文件后还原"** | 还原逻辑写在 `[Code]` 的 `InitializeUninstall()` 里，通过 `ShellExec('runas', '{app}\DelayStart.exe', '--restore-all', '', SW_SHOW, ewWaitUntilTerminated, R)` 调起，**必须 `ewWaitUntilTerminated`**（`shellexec` / `nowait` 都不等待 → 会出现"文件已删、还原未跑完"）；不得依赖已被删除的文件 | 半还原状态 + 无法修复 |
| **还原失败则中止卸载** | `R <> 0` → `InitializeUninstall()` 返回 `False`，**Inno 会中止整个卸载**，安装目录文件保留，弹窗列出失败项 | 同上 |
| **计划任务清理** | 卸载时删除 `DelayStartScheduler`；删除失败（组策略保护）必须提示用户手动删除，不得静默忽略 | 残留指向空路径的任务，每次登录都报错 |
| **安装路径落盘** | 用户允许改安装目录，但实际路径必须写进 `config.json`，供计划任务重新注册时使用 | 计划任务指向错误路径 |

#### 代价（诚实列出）

- 需要维护一个 `.iss` 脚本（约 100–150 行）+ 一个"卸载前置还原"流程。这是 D8 选 zip 时完全不存在的工作量。
- 换来的是：**计划任务路径稳定 + 安装体验正常 + Win10 可用 + 卸载不会毁掉用户的自启动配置**。这笔账没有争议。

> **已作废的内容**：`build-and-test.md` 第七节原「7.1 免安装 zip」**不再是交付形态**，降级为"本地开发自测用的目录组装步骤"保留；`NFR-6.3` 已改写。凡文档中出现「第一版免安装 zip」的地方，一律以本节为准。

---

## 6.5 D23 安装与数据布局

**用户定的三条（不可改）：** 程序不放 `Program Files`；配置走 Roaming；日志走 Local。它们背后是一个共同判断：**这是一个 per-user 的个人工具，不是机器级的系统组件**。

### 落地方案

| 项 | 值 | 理由 |
|---|---|---|
| 程序 | `%LOCALAPPDATA%\Programs\DelayStart` | per-user 安装的标准位置（VS Code / Chrome 同款）。**不含版本号** → 计划任务 action 永久有效 |
| 配置 | `%APPDATA%\DelayStart\config.json` | Roaming = 用户资产，重装系统/换机应当跟人走 |
| 日志 / 实时状态 / 运行归档 | `%LOCALAPPDATA%\DelayStart\` | Local = 机器相关产物，没有漫游价值，漫游只会造成无谓同步 |
| 安装器权限 | `PrivilegesRequired=lowest` | per-user 安装**不需要管理员**。放 `Program Files` 就必须提权安装，而提权安装与 per-user 数据目录是**互相矛盾**的组合。⚠️ 注意：**安装零 UAC，但卸载会弹一次** —— 还原 HKLM 接管项、删除计划任务必须有管理员，这一次躲不掉 |
| 计划任务注册时机 | **管理端首次启动**（`--reinstall-task` 幂等） | 注册计划任务必须提权，这是本项目唯一逃不掉的 UAC。把它放在"用户第一次打开程序"这一次，比放在"安装那一刻"更自然——**安装完还没打开过的用户，本来也不需要这个任务** |

### 为什么安装目录必须只读（NFR-6.7）

如果程序往自己的安装目录写配置，就会出现三个后果：覆盖升级时旧配置被覆盖或残留、卸载删不干净、`%LOCALAPPDATA%\Programs` 下的目录被用户当成"程序文件夹"整体删除时连带丢配置。**程序与数据分离不是洁癖，是升级/卸载/备份三条链路的前提。**

### 🔴 派生出的隐性坑：SYSTEM 身份

计划任务很容易被写成"以 SYSTEM 运行"（很多教程这么写）。**在本项目里这会静默毁掉一切**：SYSTEM 会话下 `%APPDATA%` 解析到 `C:\Windows\System32\config\systemprofile`，配置读不到、日志写到那个目录 —— 而且**不会抛任何异常**，表现为"调度器启动了但什么都没干"。

所以硬性要求（NFR-6.8）：`LogonType=Interactive` + `RunLevel=Highest`，身份 = 发起安装的交互用户。**危险等级与 MSIX 注册表虚拟化同级：都是静默失效。**

### 代价（诚实列出）

- Roaming 在域环境下会把机器相关的条目列表带到另一台机器 → 加载时必须按 `ItemKey` 校验存在性，失效条目标记「已失效」且不参与调度（复用 E3 逻辑）。这一条是**必须实现**的，不是可选优化。
- 卸载比"删一个目录"复杂：要先提权还原接管项、删计划任务，再删程序文件，最后询问是否删配置。

> **完整实现规则（6 条）见 `architecture.md` 1.5**；数据契约表见 `requirements.md` 第七节；安装器要点见 `build-and-test.md` 7.1 / 7.2。

### 6.6 ✅ D24（Phase 0 新发现，已批复 B）：调度端 UI 技术选型 —— 重开 D1

> **性质说明**：这不是新需求，是 Phase 0 把 D1 的前提打掉了。D1 定的是"调度端 = 独立轻量进程 WinForms + NativeAOT"，但当时不知道 WinForms + AOT 是**官方不支持、SDK 主动拦截**的组合。现在事实清楚了，必须让用户重新选一次。

**Phase 0 实测事实（不是推断）**

给 `DelayStart.Scheduler` 加上 `PublishAot=true` 后构建**直接失败**：

```
error NETSDK1175: 启用剪裁时，不支持或不推荐使用 Windows 窗体。
```

读 SDK 源码定位到确切条件（`Microsoft.NET.RuntimeIdentifierInference.targets:305`）：

```xml
<NetSdkError Condition="('$(UseWindowsForms)' == 'true') and ('$(PublishTrimmed)' == 'true')
                     and ('$(_SuppressWinFormsTrimError)' != 'true')"
             ResourceName="TrimmingWindowsFormsIsNotSupported" />
```

而 `PublishTrimmed` 是被 `PublishAot` 隐式打开的（`Microsoft.NET.Publish.targets`）：

```xml
<PublishTrimmed Condition="'$(PublishTrimmed)' == '' And '$(PublishAot)' == 'true'">true</PublishTrimmed>
```

**官方立场**：微软 *Known trimming incompatibilities* 明确把 WinForms / WPF 列为**不支持 trimming**；.NET 10 的发布说明同样写着 NativeAOT 对 WinForms/WPF 不可用。根因是 **Windows 上的 Native AOT 没有 built-in COM**，而 WinForms 对 built-in COM marshalling 依赖很重。

**社区现状**：`_SuppressWinFormsTrimError` 是 SDK 自己留的逃逸口，**加上后确实能编译、能产出可独立运行的 exe**，但：

- 它是**内部属性**（下划线开头，不在公开 API 文档里）—— 随时可能改名或消失。
- 它**不修复任何 trim 问题**，只是把拦截降级。社区有实测"编译通过但 exe 启动即闪退"的案例。
- 已知高危路径里，**本项目会踩到 `.resx` 反射式资源加载**（托盘图标），其余（DataGridView / RichTextBox / 动态 COM）不碰。

**先纠正一条我们自己写错的结论**

本文档 D1 行与 `api-analysis.md` 原先都写着"WinUI 3 的 `PublishAot` 仍属 preview"。**这条是错的，且已过期**：

> **Windows App SDK 1.6 起，NativeAOT 已是官方支持的正式特性。** 微软官方公告（*So what's new with Microsoft native UX technologies*，2024-11）把 Native AOT 列为 WinAppSDK 1.6 的主要新特性之一："In WinAppSDK 1.6, WinUI adds Native Ahead-of-Time compilation support"，官方示例 Contoso Camera **启动时间减少 50%、框架包体积减少约 8 倍、自包含体积减少约 2 倍**。

所以 D1 当初用来排除 WinUI 3 的**两条理由里，一条失效、一条仍成立**：

| D1 的原理由 | 现状 |
|---|---|
| "WinUI 3 的 AOT 仍是 preview" | ❌ **失效** —— 1.6 起官方支持 |
| "自包含体积大、冷启动 1–2s" | ✅ **仍成立且更硬** —— 第三方实测 WinUI 3 即使 AOT 冷启动仍约 **0.5–0.7 s**（WPF 约 0.7s / WinUI 3 约 0.5s，含 AOT），**超出 `NFR-1.2` 的 0.3 秒**；且自包含仍需带上 WinAppSDK |

**再纠正一个默认假设：WinUI 3 根本没有托盘 API**

用户问"以 WinUI 3 的推荐技术规范为准"——这个问法在本问题上**得不到答案，因为前提不存在**：

> **WinUI 3 / Windows App SDK 不提供任何系统托盘（通知区域）API，这是微软有意省略的。** 想在 WinUI 3 里放托盘图标，**唯一的路仍是 `Shell_NotifyIcon` P/Invoke**（官方口径如此，社区库 `H.NotifyIcon.WinUI` / `WinUIEx` 也只是把这套 Win32 调用包了一层）。

**也就是说：把"官方推荐"推到底，落到的正是"用 `Shell_NotifyIcon` 直接做托盘"这条路 —— 而选项 B 就是这条路，只不过它连 WinForms/WinUI 都不叠。**

**四个选项**

| 选项 | 做法 | 官方支持？ | 代价 / 收益 |
|---|---|---|---|
| **A. 维持 WinForms + AOT** | csproj 写 `_SuppressWinFormsTrimError=true`（**Phase 0 已临时这么配，否则构建过不去**） | ❌ **官方明确不支持**，靠内部属性逃逸 | 改动最小、当下能跑；但走灰产路径，SDK 一变就返工；`.resx` 写法必须全避开 |
| **B. 改纯 Win32 + AOT**（推荐） | 弃用 WinForms：`Shell_NotifyIcon` 托盘 + 自绘无边框弹窗（P/Invoke） | ✅ **完全支持** —— 纯 P/Invoke 落在 NativeAOT 的受支持范围内 | 体积最小（可到个位数 MB）、冷启动最快（能守住 `NFR-1.2` 的 0.3s）；代价是面板要自己画，多几百行 Win32 代码 |
| **C. 改 WinUI 3 + AOT** | 调度端也用 WinUI 3 | ✅ **官方支持**（WinAppSDK 1.6+） | 🔴 冷启动约 0.5–0.7s **超 NFR-1.2**；自包含体积比纯 Win32 大一两个数量级；**而且托盘仍要自己写 `Shell_NotifyIcon`** —— 为了一个只读小面板引入整套 WinAppSDK，收益为负 |
| **D. 放弃 AOT** | WinForms 走框架依赖 | ✅ 支持 | 冷启动 1–2s、体积 60MB+、还要用户装 .NET Desktop Runtime。**与 D1 立论直接冲突，最差** |

**批复结果：B ✅（2026-09-19 用户批复）**

用户在得知"WinUI 3 没有托盘 API、其 AOT 其实已官方支持"这两个纠正后，选择 **B —— 纯 Win32 + AOT**。

**Phase 0 已按 B 落地**：

| 改动 | 内容 |
|---|---|
| `DelayStart.Scheduler.csproj` | **移除** `<UseWindowsForms>true</UseWindowsForms>` 与 `<_SuppressWinFormsTrimError>true</_SuppressWinFormsTrimError>`；保留 `WinExe` + `PublishAot` + `win-x64` + `SelfContained` + `StripSymbols` |
| `Program.cs` | 模板的 WinForms 入口（`Application.Run(new Form1())`）替换为纯 Win32 入口骨架，并把五条实现铁律写进 XML 注释（WinExe / 禁 `.resx` / 禁反射与动态 COM / 通知只能用 `Shell_NotifyIcon` 气泡 / 禁止 SYSTEM 身份） |
| 构建复验 | ✅ 去掉逃逸属性后 **0 警告 0 错误** —— 直接证明"不靠内部属性也能通过"，这是 B 相对 A 的直接收益 |

**B 的论证（决策时的完整理由）**

1. **A 是被官方拒绝的路。** 靠 `_SuppressWinFormsTrimError`（下划线开头 = 内部属性、不在公开 API 文档）绕过去，等于把关键路径押在一个微软随时能改名的开关上。
2. **C 虽然官方支持，但解决不了问题还引入新问题。** 托盘 API 它没有，还是得写 `Shell_NotifyIcon`；而它带来的冷启动（0.5–0.7s）**直接违反 NFR-1.2**。既然托盘代码无论如何都得自己写，不如**只写那一层，不叠框架**。
3. **B 的需求量本来就只有几百行。** 调度端 UI = 一个托盘图标 + 悬停一行 tooltip + 点击弹出一个只读列表。为省这几百行去背一整个 WindowsDesktop / WinAppSDK，是把负债换成了"少写代码"。

**仍需做的事**：模板生成的 `Form1.cs` / `Form1.Designer.cs` 待删除（Phase 0 已在 csproj 里用 `<Compile Remove>` 临时排除，删完即可移除该块）。
**R1 照做**：Phase 4 第一件事是写最小 demo 发布 AOT，并在真机登录场景下实测托盘、面板、失焦即关三条路径。

### 6.7 ✅ D25（Phase 0 收尾，已批复 A）：模板页与导航项一并删除

**问题**：`dotnet new winui-navview` 生成的 `Pages/`（HomePage / AboutPage / SettingsPage）与 `MainWindow` 的 `Home`/`About` 两个导航项，跟我们实际的页集对不上。

`architecture.md` 1.4 定义的页集是 **OverviewPage / ItemsPage / DelayPage / SystemPage / LogPage / SettingsPage**。其中只有 `SettingsPage` 名字撞上了 —— 但模板那个是占位符，内容与 `design-spec.md` 第五节定义的设置页毫无关系。

| 选项 | 判断 |
|---|---|
| **A. 现在就删**（✅ 批复） | 模板页留着只会变成 **6 个待维护的死文件 + 一个会误导人的假导航**。Phase 3 本来就要按文档从零建 6 个页面，没有复用价值 |
| B. 留到 Phase 3 整体替换 | 省一次构建验证，代价是 Phase 3 要多做一遍"先删再建"，中间还得忍着死链导航 |

**批复结果：A。**

**已执行（Phase 0）**：

| 改动 | 内容 |
|---|---|
| `Pages/` | 6 个文件全部删除（3 组 `.xaml` + `.xaml.cs`），目录一并移除 |
| `MainWindow.xaml` | 删掉 `Home` / `About` 两个 `NavigationViewItem`；**不预置占位项** —— 点了没反应的死链比没有导航更糟；`IsSettingsVisible=False`（本产品的「设置」是 6 个顶层模块之一，不用 `NavigationView` 内置的那个）；标题由模板的 `DelayStart.App` 改为 **`DelayStart`**（D6；🔴 **D61 改判**：应用名只用英文，窗口标题 / 开始菜单与桌面快捷方式 /「应用和功能」三处统一，不再出现中文应用名） |
| `MainWindow.xaml.cs` | 去掉 `using DelayStart.App.Pages` 与 `NavView_SelectionChanged`（页面类型已不存在，留着就是编译不过或死代码） |
| 导航落地时机 | **Phase 3**：按 `architecture.md` 1.4 接入 6 个顶层模块，`自启动项` 下再按来源分子项 |

### 6.8 ⏳ D26（Phase 0 新发现，待决策）：`dotnet test` 跑不了，测试命令怎么定

**事实（Phase 0 实测，不是推断）**

| 命令 | 结果 |
|---|---|
| `dotnet run --project tests/DelayStart.Core.Tests -c Release` | ✅ `Total: 2, Errors: 0, Failed: 0`，退出码 **0** |
| `dotnet test tests/DelayStart.Core.Tests` | ❌ `运行了零个测试`，退出码 **5**（MTP 的 `ZeroTests`） |

**已排除的可能**（逐一试过，都不是原因）：

- ~~漏了 `IsTestProject=true`~~ —— 加上无效
- ~~参数没转发~~ —— `-- --list-tests` 传不进去，但也没有报"未知选项"
- ~~包版本冲突~~ —— 解析结果是 `Microsoft.Testing.Platform` **2.4.0** + `xunit.v3.mtp-v2` **4.0.1**，两侧都是 MTP v2，对齐

**而测试发现本身是好的**：同一个产物用 xUnit 自带运行器枚举一切正常（`-list tests` 有输出，两个用例都在）。
所以问题定位在 **`xunit.v3.mtp-v2` 4.0.1 与 .NET SDK 10.0.401 的 `dotnet test` 集成这一段**，不是我们的用法错了。

**选项**

| 选项 | 做法 | 代价 / 收益 |
|---|---|---|
| **A. 只改命令**（✅ 已生效，AI 建议） | 规范命令定为 `dotnet run --project tests/DelayStart.Core.Tests -c Release` | **零改动、已验证**。xUnit v3 的测试项目本来就是可执行程序，独立跑 stdout 直读、退出码语义明确；CI 里换成这条命令即可 |
| B. 补 VSTest 适配器 | 加 `xunit.runner.visualstudio` + 从 `global.json` 去掉 `test.runner: Microsoft.Testing.Platform` | 能退回 VSTest 路径，**VS 的测试资源管理器也能用**；代价是多一个包、且放弃了 MTP |
| C. 换包 | 从 `xunit.v3.mtp-v2` 换成不带 `-mtp-v2` 变体的 `xunit.v3` | 有可能直接修好 `dotnet test`；但要重新验证，属于版本轮盘 |

**AI 建议 A**，理由：A 今天就能用且已验证；B / C 都是优化，收益是"用上更顺手的命令"，**对产品功能零影响**。
**不阻塞 Phase 1** —— 测试照写，只是跑的命令不同。

**⚠️ 顺带记一笔对 VS 工作流的影响**：目前**在 VS 的测试资源管理器里看不到、也点不动这些测试**
（它走 VSTest 适配器，本项目没引用；而 MTP 路径又不通）。要用 VS 跑测试就得选 B。

---

### 6.9 ✅ D27（Phase 1 批复）：P/Invoke 启动器推到 Phase 4

**背景**：Phase 1 的交付物是 `Core` 层。而 `Core/Launch/ProcessLauncher.cs` + `TokenHelper.cs` + `Core/Interop/*`
需要移植 demo 的 `WTSQueryUserToken` / `CreateEnvironmentBlock` / `CreateProcessAsUser` 四段 P/Invoke（约 300 行）。

**为什么不本期写**

| 理由 | 说明 |
|---|---|
| **测不了就没有绿灯** | 这三条路径的正确性**只在真实登录会话里成立**：① 管理员条目继承调度端令牌；② 普通条目 `WTSQueryUserToken` 降权；③ 降权失败回退 `Process.Start`。Phase 1 的调度端还不存在（Phase 4 才做），写了只能靠"编译通过 + 代码审阅"自证，**这是假绿灯**，比不写更危险 |
| **会污染 Phase 1 的出口条件** | Phase 1 的出口是"单元测试全绿"。P/Invoke 无论怎么写都进不了单元测试（`coding-standards.md` 14.1 要求测试不碰真实注册表 / 文件 / 进程），留下来就是一个**永远无法勾选的待办** |
| **接口留白就够了** | `IProcessLauncher` 是抽象，`LaunchResultEvaluator` / `LaunchOutcome` 是纯逻辑，**这三者才是 Phase 1 该交付的**——它们把"什么算启动成功"钉死（机制 7），Phase 4 只负责把真实进程塞进去 |

**批复：A。** 本期交付 `IProcessLauncher`（接口）+ `LaunchOutcome` / `LaunchEvaluation`（结果模型）+ `LaunchResultEvaluator`（判定逻辑 + 假探针测试）。
`ProcessLauncher` / `TokenHelper` / `Core/Interop/*` 在 `architecture.md` 2.1 中标 ⏳ Phase 4。

> **代价与兜底**：Phase 4 开工第一件事仍是 R1（AOT 产物真机实测），届时 P/Invoke 与调度引擎一起落地并**在同一次真机重启中验证**，比 Phase 1 写完放到 Phase 4 再验更集中。

### 6.10 ✅ D28（Phase 1 批复）：UWP 激活弃 COM，改 `shell:AppsFolder`

**事实（Phase 1 查证，非推断）**

demo 用 `CoCreateInstance(CLSID_ApplicationActivationManager)` + `[ComImport]` 接口 + `Marshal.GetObjectForIUnknown` 激活 UWP。
在 NativeAOT 下这条路**必然失败**：NativeAOT **没有 built-in COM**，`Marshal.GetObjectForIUnknown` 会抛
`PlatformNotSupportedException`（构建期表现为 **IL3052**，而 `Core.csproj` 已开 `IsAotCompatible=true`）。

根因与 R11 同源 —— WinForms 之所以不能 AOT，也是因为它重度依赖 built-in COM marshalling（`architecture.md` R11）。
而 **D24 = B 已把调度端钉死在 AOT 上**，所以 B 选项在架构层面不可能成立。

**选项**

| 选项 | 做法 | 判定 |
|---|---|---|
| **A. `shell:AppsFolder`**（✅ 批复） | `Process.Start("explorer.exe", $@"shell:AppsFolder\{aumid}")` | 零 COM、零 P/Invoke，AOT 下可用；一行代码 |
| B. 维持 COM 激活 | demo 原样移植 | ❌ AOT 下运行时崩，与 D24=B 冲突 |
| C. 砍掉 UWP 延时 | 回退 D4 的 A 选项 | ❌ 推翻已批复的 D4，且 UWP 项占自启动项相当比例 |

**批复：A。**

**A 的代价必须写清（这是取舍，不是零成本）**

| 代价 | 处理 |
|---|---|
| **拿不到 PID** —— `explorer.exe` 立刻退出，它拉起的 UWP 应用进程无从对应 | 机制 7 的"1.5 秒后复查 `HasExited`"**对 UWP 不适用**。`LaunchResultEvaluator.Evaluate` 收到 `null` 快照时**直接判成功**（"已交给系统，无从复核"），不再套退出码规则 |
| **绕不过系统启动管理** | 这是 D4 已批复并已写进 UI 文案的既定事实（`requirements.md` 4.2），本条不新增 |
| **UWP 是唯一"延时 + 绕过启动管理"并存的来源** | UI 必须在 UWP 条目上单独标注，不能与注册表项共用一套文案 |

> **登记新风险 R12**（`architecture.md` 第九节）：AUMID 的取得方式（`Get-StartApps` / `PackageManager`）、
> 以及 `shell:AppsFolder` 在**提升权限进程**中是否仍能激活（UIPI 相关）—— 后者 Phase 4 真机实测覆盖。

### 6.11 ✅ D30–D34（Phase 2 开工前批复）：范围界定与一处文档错误修正

Phase 2 是 Management 层，第一次真正**写系统**（注册表 / 启动文件夹 / 计划任务 / UWP 状态）。
开工前提了五个决策点，其中 **D31 不是新功能，是文档里的真错误**。

#### 6.11.1 D30：本期**不做**的三件事

`IconProvider` / `ServiceQueryService` / `SystemStartupInspector` 推到 Phase 3（图标）与 Phase 5（系统启动项只读页）。

三条理由，按重要性排序：

1. **没有消费者**。图标是纯 UI 依赖，Phase 2 没有 UI；FR-7 的只读检查页属 Phase 3。
2. **进不了单元测试**（`coding-standards.md` 14.1）。三者都直接依赖真实系统状态（`IShellItemImageFactory` / SCM / 注册表），
   无法用假实现确定性覆盖 —— 写了只能靠"编译通过"自证，**这正是 D27 拒绝在 Phase 1 写 P/Invoke 启动器的同一条逻辑**。
3. **它们不构成任何验收项的阻塞**。9.1 / 9.3 的验收点全部落在四个 Source 与接管链路上。

> 换句话说：这两期的判据是**「能不能被自动化验证」**，不是「重不重要」。
> 它们重要，但此刻写出来只是无证据的代码。

#### 6.11.2 D31：`FailureStreakService` 的归属 —— 文档三处互相矛盾

这是 Phase 2 提案里唯一一个**必须改文档**的问题，因为按原文实现不出来：

| # | 文档原话 | 位置 |
|---|---|---|
| 1 | 提醒三级升级含「**托盘角标**」≥3 次不自动消失 | `scheduler-design.md` 9.2 |
| 2 | 托盘是**调度端**的（无主窗口、仅托盘） | D14 / `architecture.md` 六 |
| 3 | 调度端**只引用 Core**（硬边界，写在分层表与硬约束里） | `architecture.md` 1.2 / `README.md` |
| 4 | 「连续失败次数由**管理端**聚合计算」 | `scheduler-design.md` 339 行 |

1 + 2 要求在调度端起角标；3 说调度端只能看见 Core；4 却把计算放在 Management ——
而**登录那一刻管理端根本没在运行**，它不可能算出调度端要用的数。

**正解：把计算搬进 Core。**

```
连续失败次数 = f(RunRecord 列表)      // 纯函数，只读归档，不碰系统
```

- 满足 3：Core 是两端唯一共享层，调度端能拿到
- 满足 1 + 2：调度端每次**现算**（无状态，不维护计数器），与原设计"不存计数"的意图一致
- 满足 4 的**本意**：只是把"谁来算"从管理端扩到两端，而非推翻它

选它还有一个附带好处：它是**纯逻辑**，可以注入假的 `RunRecord` 列表做确定性测试 ——
若留在 Management 且依赖真实 `runs/` 目录，测试就得操作文件系统。

> ⚠️ 这与 `RunRecord` 的注释直接冲突（原文写"由 **Management 层的** `FailureStreakService`"），
> 该注释在本次一并更正。

#### 6.11.3 D32：headless CLI —— 让 Phase 2 的产物**能被真实执行**

Phase 2 出口条件是 `build-and-test.md` 9.1 的**全量快照对比 —— 三个 `Run` 键的值数量与内容零变化**（= `requirements.md` 9.3 第 1 条）。
但 `TakeoverService` 的调用方是管理端 UI，UI 在 Phase 3 —— 于是验收点变成**无入口可达**。

选 B（推到 Phase 3）的代价是滚雪球：Phase 2 写完全部写入链路却一次都没跑过，
等到 Phase 3 才第一次碰真实注册表，那时**栈已经有三层深**，出错无法定位是哪一层的假设错了。

**A 的四条命令，其中两条本来就必须写**：

| 命令 | 用途 | 出处 |
|---|---|---|
| `--restore-all` | 还原全部接管项 + 返回退出码 | **D22 卸载可逆性必需**（Inno `[Code] InitializeUninstall()` + `ewWaitUntilTerminated` + 非 0 中止卸载） |
| `--reinstall-task` | 幂等注册/更新 `DelayStartScheduler` | **D22「管理端首启幂等注册」的同一入口** |
| `--takeover` / `--release` | 最小接管/释放，供验证据 | 本期新增，几十行 |

> 🔴 `--restore-all` **必须**返回有意义的退出码：`[UninstallRun]` 读不到退出码，
> 所以卸载还原只能走 `[Code]` + `ewWaitUntilTerminated`。
> 违反的后果是用户卸载后所有程序**永久不自启且不知情** —— 本项目最严重的潜在缺陷。

#### 6.11.4 D33：Management 层单测范围

两条纯编排逻辑，用假实现可确定性覆盖：

| 需求 | 怎么测 | 为什么手工测不出来 |
|---|---|---|
| **FR-1.4** 单条读取失败不影响整次扫描 | 假 `IStartupSource` 中一个实例 `Scan()` 抛异常，断言其余 Source 的结果仍然汇总、异常被记日志 | 真机上很难**按需**造出"某一个计划任务读取抛异常" |
| **FR-3.1** 任一步失败逆序回滚 | 假 Store 让第 3 步抛异常，断言第 1、2 步已被撤销（配置里没有半成品条目、系统项未被禁用） | 这是"失败路径"，正常操作永远走不到 |

真机写入链路（12 字节标记、`Run32` 分流、三级回退）仍由 **D32 的 CLI + 注册表导出对比**覆盖 —— 两层互补，不重复。

#### 6.11.5 D34：计划任务本期验证

验证**不需要调度端能跑**。`schtasks /query /tn DelayStartScheduler /v /fo LIST` 直接可读：

| FR-11.1 要素 | 在 `schtasks` 输出里的位置 |
|---|---|
| 登录触发 | `计划类型: 登录时` / `Logon Mode` |
| 延迟 3s | 触发器内的 `延迟` / `Delay` 字段 |
| `RunLevel=Highest` | `以最高权限运行` / `运行级别` |
| 🔴 **身份是当前交互用户** | `运行身份` / `作为用户` —— 必须**不是** `SYSTEM` |

**为什么不能推后**：SYSTEM 身份下 `%APPDATA%` 解析到
`C:\Windows\System32\config\systemprofile`，配置读不到、日志写错位置，**且不抛任何异常**（静默失效）。
这是全项目最容易静默出错的一处，拖到最后一个阶段意味着它可能一直没被验证过。

> 注：`api-analysis.md` 3.1 要求用 `TaskService` API 而非 `schtasks.exe` 创建（FR-11.3），
> 但**验证**用 `schtasks` 只读查询反而更可信 —— 它是独立的第三方视角，不依赖本程序自己的代码解释结果。

---

### 6.12 ✅ D35–D37（Phase 3 开工前批复）

#### 6.12.1 D35：管理端 DI 容器

`architecture.md` 5.3 的启动流程只写了"构建 DI 容器"，没指定实现。管理端要装配约 20 个对象：
7 个 Source 实例（注册表 ×3 / 启动文件夹 ×2 / 计划任务 ×1 / UWP ×1）、`ScanService`、`TakeoverService`、
`TaskRegistrationService`、`ShellLinkResolver`、`StartupApprovedStore`、以及 6 个 ViewModel。

三条路的差别不在"能不能跑"，而在**依赖是否可见**与**生命周期是否显式**：

| 选项 | 致命处 |
|---|---|
| B 手写组合根 | `AppServices` 是个静态类，ViewModel 想拿服务只能 `AppServices.Scan` 这样**静态取用**。依赖从构造函数签名里消失，变成藏在方法体里的全局访问 —— 漏注册一个服务是**运行时空引用**，编译期毫无提示 |
| C `CommunityToolkit.Mvvm` 的 `Ioc` | 它只是包在 `IServiceProvider` 外面的静态壳，**没有生命周期概念**。本项目需要"`ConfigStore` 单例 / Scanner 复用 / Dialog 瞬态"这三档区分，用它等于自己再写一层 |
| **A `Microsoft.Extensions.DependencyInjection`** | 官方包。构造注入让依赖在类型签名上可见；`Singleton` / `Transient` 显式声明；与两端已有的 `IStartupSource` / `IAppConfigStore` 接口抽象天然契合 |

选 A。代价是 1 个直接依赖 + 2 个传递依赖。

#### 6.12.2 D36：导出/导入与预设延时值不提前

两者的正式归宿都是 **Phase 5 的设置页**：FR-9.10 的「数据分组」把 `打开配置目录 / 导出配置 / 导入配置 / 重置`
放在同一处，而 FR-12.2 的导入还需要"版本校验 + 差异预览"—— 这跟设置页的那套是同一个组件。
单独提前做，Phase 5 再接到设置页时要改一次。

「预设延时值可编辑」（D11=B）同理：Phase 3 的延时编辑器**已经会从 `AppConfig` 读预设值**，
本期只是没有编辑入口 —— 用默认值 `10/30/60/120` 秒即可，**不影响本期任何交互**。
把设置页的一半提前做出来，只会得到一个半成品页面。

#### 6.12.3 D37：时间轴整体删除（**需求变更**）

用户批复"**两处都砍**"。这是**需求变更**，不是实现选择，因此同步修改了四处：

| 位置 | 改动 |
|---|---|
| 页面 1（总览） | 删「开机启动时间轴」卡片 |
| 页面 3（延时启动） | 删时间轴视图 + 工具栏 `[列表 \| 时间轴]` 分段控件，改称"列表" |
| `requirements.md` FR-10.4 | "时间轴或进度视图" → **"进度视图"** |
| `architecture.md` 5.1 | 目录结构删 `Controls/TimelineView.xaml` |

**D7 的措辞一并对齐**："0.5 倍速播放时间轴" → "0.5 倍速播放进度视图"。
「立即模拟调度」这个功能本身不变（仍是真的按延时启动程序），只是可视化形式从时间轴改成进度列表。

**这一刀砍掉了什么**：延时启动页原来有一套「双视图 + 搜索态同步过滤（时间轴也要跟着过滤，无匹配时还要给出
`没有匹配的条目。清空搜索框可恢复完整时间轴。`）+ 上下调序与过滤态互斥」的分支 —— 这是本页最容易长 bug 的地方，
因为同一条数据要在两种渲染下保持一致。现在只剩列表一种渲染。

**这一刀没有砍掉什么**：「有依赖关系就给 B 设更大的延时值」这条语义仍在（常驻提示条承担）；
「开机加载窗口」这个统计值仍在（总览页统计卡）；总览页「最近一次开机调度」表仍承担"上次启动发生了什么"。

> 原型 `docs/ui-mockup.html` 里的时间轴节点需同步删除，随 Phase 3 的文档提交一并处理。

---

## 七、本轮评审反馈的处理

### 7.1 自启动项页去掉「手动添加」✅ 已改

「手动添加」只保留在「延时启动」页。自启动项页的职责是「列出系统里已有的东西」，不是创造新东西。

### 7.2 设置页去掉两项配置 ✅ 已改

- 删除「启动时自动扫描自启动项」开关 → 改为**固定行为**：进入程序即扫描一次。
- 删除「关闭窗口时」配置 → 关闭即退出是已确认的设计，没有配置的必要。
- 整个「常规」分组随之删除，改为设置页顶部的一条固定说明。

### 7.3 疑问 1：延时时间怎么选？

**答：预设快选 + 自定义输入，两者都要；预设值本身可在设置里改。**

| 方案 | 判断 |
|---|---|
| 只给定死几个值 | ❌ 不行。延时是强场景相关的 —— 有人要 5 秒，有人要 15 分钟 |
| 只给自由输入框 | ❌ 不行。多一步敲键盘，90% 的情况都是 10/30/60 秒 |
| **预设按钮 + 自定义输入框** | ✅ 主路径一次点击，兜底任意数值 |
| **再加：预设值可在设置里编辑** | ✅ 成本极低，解决个性化 |

所以弹窗长这样：`[登录后立即] [10 秒] [30 秒] [60 秒] [2 分钟]` + `自定义 [__] 秒`，右下角一行小字 `预设值可在「设置」里改`。

**不做**：完整的多选/拖拽式预设编辑器 —— 过度设计，一个逗号分隔的文本框足够。

### 7.4 疑问 2：最大延时上限 —— 你说得对，我的设计有缺陷 ✅ 已改

**原设计的错误**：把「最大延时上限」做成了 10 分钟，这是**用配置去限制调度能力**。用户想设 1 小时延时，软件把它卡在 10 分钟 —— 那就不是"延时启动"，是"启动失败"。这个设置项存在的意义被搞反了。

**修正为两件事，彻底分开：**

| 概念 | 性质 | 归属 |
|---|---|---|
| **单条目延时上限** | 纯输入校验，防呆（防止误填 999999） | 设置项，默认 24 小时，可选"不限制" |
| **调度器驻留时长** | **算出来的**，不是设出来的 | 只读展示 = `max(所有条目的 delaySeconds)` + 面板保留时间 + 启动开销 |

举例（当前配置）：
```
SyncTool         10 秒
微信              30 秒
Epson Scanner    60 秒   ← 最长延时
AutoHotkey       60 秒
─────────────────────────
调度器驻留      60 + 3 + 2 ≈ 1 分 05 秒
```

设置页里这一行是**只读的**，并明确写「调度器会在最后一项启动后自动退出，不会常驻后台」。用户设 1 小时延时，驻留时长就是 1 小时，软件不会、也不该拦着 —— 但要让用户事先知道。

**唯一需要用户配置的"时间"** 是：全部启动完成后进度面板再保留几秒（默认 3 秒）。这才是真正需要人来定的那个值。

### 7.5 搜索框从导航栏移到页面内 ✅ 已改

原设计的错误：导航栏顶部一个全局搜索框，用户在任何页面打字都会被强制跳转到「自启动项」页。**抢走用户的上下文**是明确的 UX 缺陷，不做辩解。

改为两处**页面内搜索**（自启动项页 / 延时启动页），各自独立、只过滤当前页、不跳页、不改来源筛选。完整规则见第二节末「搜索框为什么从导航栏挪进页面」。

放弃全局搜索的理由：本程序总共十几到几十条记录，且导航栏已经按来源分好组、每项带计数。为了"快速定位某程序"而引入一个会抢焦点的全局控件，收益不抵成本。

### 7.6 手动添加 / 编辑弹窗重做 ✅ 已改

原实现只是个 Toast 占位（`原型未展开`），等于没有设计。现在补全，并按「有没有系统来源」分成 3 种形态，共用一套布局：

- **4 个编号步骤**（目标程序 / 延时时间 / 启动身份 / 系统影响或接管对象），把长表单结构化
- **目标程序块顶部是 tab**（`程序` / `UWP 应用`），两个页签各存各的目标值、各自摆该有的字段（D53）
- **手动条目和系统条目在第 4 块和第 1 块上明确分叉**：手动条目能换目标、写明"不动系统任何东西"；系统条目路径只读、写明"写入软禁用标记，可还原"
- **底部活摘要**已删除（D53）：它是上方两个胶囊的复述，不提供新信息

完整规范见第三节之二。

同时落定的数据边界：**手动添加的条目不属于系统自启动项，不进入自启动项页**。它只在「延时启动」页出现，不参与"已扫描自启动项"的计数。

---

## 八、评审通过后的实施顺序

**完整的 Phase 划分与出口条件以 `architecture.md` 第十节为准**（Phase 0–6），本节不重复。

本节只记录**与界面相关的技术验证项**——这些是界面能否按本设计落地的前提：

| 时机 | 验证项 | 不通过时的降级 |
|---|---|---|
| **Phase 0** | 🔴 **R9**：`WindowsAppSDKSelfContained=true` 时 `app.manifest` 的 `requireAdministrator` 是否生效 | 按 `architecture.md` R9 三条降级路径（① 去自包含改框架依赖 / ② 运行时 `runas` 自重启 / ③ 外部 AOT 启动器） |
| **Phase 0** | R3：unpackaged 下 `app.manifest` 生效，`PerMonitorV2` DPI 声明生效 | — |
| **Phase 3** | 🟡 **R10**：提权窗口下 `[浏览…]` 必然可用；拖放能否补上 | 拖放区降级为纯按钮，**不留一个"拖进去没反应"的控件** |
| **Phase 3 / 5** | R7：100% / 150% / 200% DPI 下无错位、无文字截断 | — |
| **Phase 4** | R1：NativeAOT 下无边框弹出面板能否工作（`Deactivate` 失焦即关） | 用 `ContextMenuStrip` 承载面板内容（功能等价、视觉弱一些、零风险），见 `scheduler-design.md` 第六节 |

**模拟调度（D7）的验收点**：不等重启就能验证延时配置是否正确。这是开发期的必需品，也是给用户的预览能力，Phase 6 打包前必须可用。

**界面文案验收**：Phase 5 出口要求全部页面文案与本文件逐字对齐。
