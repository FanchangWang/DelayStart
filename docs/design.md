...
| FR-15.13 | 内置 5 档**不可改、不可删**；自定义周期可重命名、可改星期、可删除。 |
...
- **延时编辑弹窗**：在「延时时间」与「启动身份」之间插一节「调度周期」—— 内置 5 档胶囊 + 「我的周期」+「＋ 新建周期」；选中后下方**只跟一行纯文字**说明包含哪些天（FR-15.23）。
...
- **设置页**：在「延时」与「调度」之间新增「**周期**」「**节假日数据**」两节（D92）。周期节 = 固定一张「介绍 + 新建」边框 + 每个自定义周期一张独立边框（内置 5 档不出现）；节假日节 = 每个年份一张卡片（`N 放假日 / M 补班日` + 重新下载 + 导出）+ 补齐缺失年份 + 自动检查开关 + 从文件导入 + 打开数据目录。
...
- **自动检查的三条硬要求**：① 降级链**必须覆盖超时**（超时与 HTTP 错误、空内容一视同仁地换下一个地址）；② 节流的键是**上次尝试的结局**而非"我试过了"（成功 / 未公布 7 天、失败 1 小时）；③ 后台跑的事**必须看得见**（进度上报共享状态 + 设置页副标题报事实 + 开关打开即触发一次 + 后台补齐成功发一条通知）。
